...
#include "pair_coul_long_SPH_nopseudo_ph.h"
#include <mpi.h>
#include <cmath>
#include <cstring>
#include "atom.h"
...
PairCoulLongNopseudoSPHph::PairCoulLongNopseudoSPHph(LAMMPS *lmp) : Pair(lmp)
{
...
PairCoulLongNopseudoSPHph::~PairCoulLongNopseudoSPHph()
{
  if (copymode) return;
...
void PairCoulLongNopseudoSPHph::compute(int eflag, int vflag)
{
  int i,j,ii,jj,inum,jnum,itable,itype,jtype;
  double qtmp,xtmp,ytmp,ztmp,delx,dely,delz,ecoul,fpair;
  double fraction,table;
  double r,r2inv,forcecoul,factor_coul;
  double grij,expm2,prefactor,t,erfc,erf_kspace,erf_kspace_fact,coul_prefact;
      ...
      if (rsq < cut_coulsq) {
        ...
        if (jtype != ion_species){
          ...
          if (itype == ion_species){
            ...
            coul_prefact = factor_coul*qqrd2e*scale[itype][jtype]*qtmp*q[j];

            ecoul = coul_prefact*erf(r/(sqrt2*h_j))/r;

            // remove erf coul

            ecoul -= coul_prefact*erf_kspace/r;
            ...
            erf_kspace_fact = (coul_prefact/r)*(erf_kspace/(r*r) - EWALD_F*g_ewald*expm2/r);

            f[i][0] -= (delx)*erf_kspace_fact;
            f[i][1] -= (dely)*erf_kspace_fact;
            f[i][2] -= (delz)*erf_kspace_fact;
            ...
            if (newton_pair || j < nlocal) {
              ...
              f[j][0] += delx*erf_kspace_fact;
              f[j][1] += dely*erf_kspace_fact;
              f[j][2] += delz*erf_kspace_fact;
            }

            if (evflag) ev_tally(i,j,nlocal,newton_pair,
                                 0.0,ecoul,force_fact-erf_kspace_fact,delx,dely,delz);
          }
          ...
          if (itype != ion_species){
            ...
            full_factor = force_fact - erf_kspace_fact + 0.5*(ij_fact+ji_fact);
...
void PairCoulLongNopseudoSPHph::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairCoulLongNopseudoSPHph::coeff(int narg, char **arg)
{
...
void PairCoulLongNopseudoSPHph::init_style()
{
  if (!atom->q_flag)
...
double PairCoulLongNopseudoSPHph::init_one(int i, int j)
{
  scale[j][i] = scale[i][j];
...
void PairCoulLongNopseudoSPHph::write_restart(FILE *fp)
{
...
void PairCoulLongNopseudoSPHph::read_restart(FILE *fp)
{
...
void PairCoulLongNopseudoSPHph::write_restart_settings(FILE *fp)
{
...
void PairCoulLongNopseudoSPHph::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
// double PairCoulLongNopseudoSPHph::single(int i, int j, int /*itype*/, int /*jtype*/,
//                             double rsq,
...
void *PairCoulLongNopseudoSPHph::extract(const char *str, int &dim)
{
...
int PairCoulLongNopseudoSPHph::pack_forward_comm(int n, int *list, double *buf,
...
void PairCoulLongNopseudoSPHph::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
...
int PairCoulLongNopseudoSPHph::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairCoulLongNopseudoSPHph::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
