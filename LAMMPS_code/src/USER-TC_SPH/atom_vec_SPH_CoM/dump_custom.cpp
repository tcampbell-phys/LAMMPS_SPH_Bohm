...
enum{ID,MOL,PROC,PROCP1,TYPE,ELEMENT,MASS,
     ...
     X_COM,Y_COM,Z_COM,VX_COM,VY_COM,VZ_COM};
  ...
  if (nthresh) {
    ...
    for (int ithresh = 0; ithresh < nthresh; ithresh++) {
      ...
      if (thresh_array[ithresh] == ID) {
        ...
        ptr = atom->q;
        nstride = 1;
      } else if (thresh_array[ithresh] == RHO_SPH) {
        if (!atom->TC_SPH_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = atom->rho_SPH;
        ...
        ptr = atom->width_SPH;
        nstride = 1;
      } else if (thresh_array[ithresh] == OMEGA_SPH) {
        if (!atom->TC_SPH_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = atom->omega_SPH;
        ...
        ptr = atom->u_SPH;
        nstride = 1;
      } else if (thresh_array[ithresh] == X_COM) {
        if (!atom->TC_X_CoM_SPH_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = atom->x_COM;
        nstride = 1;
      } else if (thresh_array[ithresh] == Y_COM) {
        if (!atom->TC_X_CoM_SPH_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = atom->y_COM;
        nstride = 1;
      } else if (thresh_array[ithresh] == Z_COM) {
        if (!atom->TC_X_CoM_SPH_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = atom->z_COM;
        nstride = 1;
      } else if (thresh_array[ithresh] == VX_COM) {
        if (!atom->TC_V_CoM_SPH_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = atom->vx_COM;
        nstride = 1;
      } else if (thresh_array[ithresh] == VY_COM) {
        if (!atom->TC_V_CoM_SPH_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = atom->vy_COM;
        nstride = 1;
      } else if (thresh_array[ithresh] == VZ_COM) {
        if (!atom->TC_V_CoM_SPH_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = atom->vz_COM;
...
int DumpCustom::parse_fields(int narg, char **arg)
{
  ...
  for (int iarg = 0; iarg < narg; iarg++) {
    ...
    if (strcmp(arg[iarg],"id") == 0) {
      ...
      vtype[i] = Dump::DOUBLE;
    } else if (strcmp(arg[iarg],"rho_SPH") == 0) {
      if (!atom->TC_SPH_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      ...
      vtype[i] = Dump::DOUBLE;
    } else if (strcmp(arg[iarg],"x_COM") == 0) {
      if (!atom->TC_X_CoM_SPH_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_x_COM;
      vtype[i] = Dump::DOUBLE;
    } else if (strcmp(arg[iarg],"y_COM") == 0) {
      if (!atom->TC_X_CoM_SPH_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_y_COM;
      vtype[i] = Dump::DOUBLE;
    } else if (strcmp(arg[iarg],"z_COM") == 0) {
      if (!atom->TC_X_CoM_SPH_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_z_COM;
      vtype[i] = Dump::DOUBLE;
    } else if (strcmp(arg[iarg],"vx_COM") == 0) {
      if (!atom->TC_V_CoM_SPH_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_vx_COM;
      vtype[i] = Dump::DOUBLE;
    } else if (strcmp(arg[iarg],"vy_COM") == 0) {
      if (!atom->TC_V_CoM_SPH_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_vy_COM;
      vtype[i] = Dump::DOUBLE;
    } else if (strcmp(arg[iarg],"vz_COM") == 0) {
      if (!atom->TC_V_CoM_SPH_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_vz_COM;
      vtype[i] = Dump::DOUBLE;
...
int DumpCustom::modify_param(int narg, char **arg)
{
  ...
  if (strcmp(arg[0],"thresh") == 0) {
    ...
    //TC Bohm SPH CoM
    else if (strcmp(arg[1],"x_COM") == 0) thresh_array[nthresh] = X_COM;
    else if (strcmp(arg[1],"y_COM") == 0) thresh_array[nthresh] = Y_COM;
    else if (strcmp(arg[1],"z_COM") == 0) thresh_array[nthresh] = Z_COM;
    else if (strcmp(arg[1],"vx_COM") == 0) thresh_array[nthresh] = VX_COM;
    else if (strcmp(arg[1],"vy_COM") == 0) thresh_array[nthresh] = VY_COM;
    else if (strcmp(arg[1],"vz_COM") == 0) thresh_array[nthresh] = VZ_COM;
...
void DumpCustom::pack_rho_SPH(int n)
{
...
void DumpCustom::pack_width_SPH(int n)
{
  double *width_SPH = atom->width_SPH;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = width_SPH[clist[i]];
    n += size_one;
  }
}
...
void DumpCustom::pack_omega_SPH(int n)
{
...
void DumpCustom::pack_x_COM(int n)
{
...
void DumpCustom::pack_y_COM(int n)
{
...
void DumpCustom::pack_z_COM(int n)
{
...
void DumpCustom::pack_vx_COM(int n)
{
  double *vx_COM = atom->vx_COM;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = vx_COM[clist[i]];
    n += size_one;
  }
}
...
void DumpCustom::pack_vy_COM(int n)
{
  double *vy_COM = atom->vy_COM;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = vy_COM[clist[i]];
    n += size_one;
  }
}
...
void DumpCustom::pack_vz_COM(int n)
{
  double *vz_COM = atom->vz_COM;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = vz_COM[clist[i]];
    n += size_one;
  }
}
