  ...
  for (int iarg = 3; iarg < narg; iarg++) {
    ...
    if (strcmp(arg[iarg],"id") == 0) {
      ...
      if (!atom->TC_SPH_flag)
        error->all(FLERR,"Compute property/atom for "
                   "atom property that isn't allocated");
      pack_choice[i] = &ComputePropertyAtom::pack_width_SPH;
      ...
      if (!atom->TC_X_CoM_SPH_flag)
        error->all(FLERR,"Compute property/atom for "
                   "atom property that isn't allocated");
      pack_choice[i] = &ComputePropertyAtom::pack_x_COM;
    } 
    else if (strcmp(arg[iarg],"y_COM") == 0) {
      if (!atom->TC_X_CoM_SPH_flag)
        error->all(FLERR,"Compute property/atom for "
                   "atom property that isn't allocated");
      pack_choice[i] = &ComputePropertyAtom::pack_y_COM;
    } 
    else if (strcmp(arg[iarg],"z_COM") == 0) {
      if (!atom->TC_X_CoM_SPH_flag)
        error->all(FLERR,"Compute property/atom for "
                   "atom property that isn't allocated");
      pack_choice[i] = &ComputePropertyAtom::pack_z_COM;
    }
    else if (strcmp(arg[iarg],"vx_COM") == 0) {
      if (!atom->TC_V_CoM_SPH_flag)
        error->all(FLERR,"Compute property/atom for "
                   "atom property that isn't allocated");
      pack_choice[i] = &ComputePropertyAtom::pack_vx_COM;
    } 
    else if (strcmp(arg[iarg],"vy_COM") == 0) {
      if (!atom->TC_V_CoM_SPH_flag)
        error->all(FLERR,"Compute property/atom for "
                   "atom property that isn't allocated");
      pack_choice[i] = &ComputePropertyAtom::pack_vy_COM;
    } 
    else if (strcmp(arg[iarg],"vz_COM") == 0) {
      if (!atom->TC_V_CoM_SPH_flag)
        error->all(FLERR,"Compute property/atom for "
                   "atom property that isn't allocated");
      pack_choice[i] = &ComputePropertyAtom::pack_vz_COM;
    } 
...
void ComputePropertyAtom::pack_rho_SPH(int n)
{
...
void ComputePropertyAtom::pack_width_SPH(int n)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) buf[n] = width_SPH[i];
    else buf[n] = 0.0;
    n += nvalues;
  }
}
...
void ComputePropertyAtom::pack_omega_SPH(int n)
{
...
void ComputePropertyAtom::pack_x_COM(int n)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) buf[n] = x_COM[i];
    else buf[n] = 0.0;
    n += nvalues;
  }
}
...
void ComputePropertyAtom::pack_y_COM(int n)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) buf[n] = y_COM[i];
    else buf[n] = 0.0;
    n += nvalues;
  }
}
...
void ComputePropertyAtom::pack_z_COM(int n)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) buf[n] = z_COM[i];
    else buf[n] = 0.0;
    n += nvalues;
  }
}
...
void ComputePropertyAtom::pack_vx_COM(int n)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) buf[n] = vx_COM[i];
    else buf[n] = 0.0;
    n += nvalues;
  }
}
...
void ComputePropertyAtom::pack_vy_COM(int n)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) buf[n] = vy_COM[i];
    else buf[n] = 0.0;
    n += nvalues;
  }
}
...
void ComputePropertyAtom::pack_vz_COM(int n)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) buf[n] = vz_COM[i];
    else buf[n] = 0.0;
    n += nvalues;
  }
}
