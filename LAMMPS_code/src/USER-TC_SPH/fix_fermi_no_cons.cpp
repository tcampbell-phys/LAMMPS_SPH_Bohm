...
{
  if (narg < 5) error->all(FLERR,"Illegal fix Fermi command");
  ...
  alpha_mass = force->numeric(FLERR,arg[4]);
  species_mass = force->numeric(FLERR,arg[5]);
  exp_cutoff = force->numeric(FLERR,arg[6]);
  mu = force->numeric(FLERR,arg[7]);
...
void FixFermiNoCons::initial_integrate(int /*vflag*/)
{
	...
	for(int i = 0; i < nlocal; ++i){
		if (mask[i] & groupbit){
			...
			particle_energy *= (0.5 * species_mass * force->mvv2e);
	...
	alpha_plus_quarter = alpha_minus_quarter + alpha_dot * dtv_2;
	...
	for(int i = 0; i < nlocal; ++i){
		if (mask[i]&groupbit){
			...
			for(int j = 0; j < ndim; ++j){
				...
				v_plus_quarter[i][j] = v_minus_quarter[i][j] + dtf * (potential_force + friction_force) / species_mass;
				v[i][j] = 0.5 * (v_minus_quarter[i][j] + v_plus_quarter[i][j]);
				x[i][j] += dtv * v_plus_quarter[i][j];
			}
		}
	}
}
