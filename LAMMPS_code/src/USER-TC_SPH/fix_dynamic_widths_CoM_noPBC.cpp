...
FixDynamicWidthsCoMNoPBC::FixDynamicWidthsCoMNoPBC(LAMMPS *lmp, int narg, char **arg):
	Fix(lmp, narg, arg), mu(NULL), mu_all(NULL), tau(NULL), tau_all(NULL)
{
  ...
  assignment_flag = 1;
  // fprintf(screen,"\nIn FixDynamicWidthsCoMNoPBC with MPI_Allreduce and no commflag = 4 or 5");
}

FixDynamicWidthsCoMNoPBC::~FixDynamicWidthsCoMNoPBC()
{
...
int FixDynamicWidthsCoMNoPBC::setmask()
{
  int mask = 0;
...
void FixDynamicWidthsCoMNoPBC::init()
{ 
...
void FixDynamicWidthsCoMNoPBC::setup_pre_force(int)
{
  FixedPointIterator();
  CoM_Calculator();
}

void FixDynamicWidthsCoMNoPBC::min_pre_force(int)
{
  FixedPointIterator();
  CoM_Calculator();
}
...
void FixDynamicWidthsCoMNoPBC::setup_post_neighbor()
{
...
void FixDynamicWidthsCoMNoPBC::post_neighbor()
{
  return;
}

void FixDynamicWidthsCoMNoPBC::pre_force(int)
{
  FixedPointIterator();
  CoM_Calculator();
}

void FixDynamicWidthsCoMNoPBC::FixedPointIterator()
{
...
double FixDynamicWidthsCoMNoPBC::Gauss_Width_Deriv(double pre_fact, double wid, double sep_sq)
{
  return pre_fact*exp(-sep_sq/(wid*wid*2.0))*((sep_sq)/(wid*wid*wid) - 3.0/wid);
}

void FixDynamicWidthsCoMNoPBC::CoM_Calculator()
{
  int i,ii,inum;
  int *ilist;
	double **x = atom->x;
  ...
  if (unallocated){
    deallocate();
    // // fprintf(screen,"\nFixDynamicWidthsCoMNoPBC allocating mu tau memory...\n");
    allocate();
    allocated = 1;
    unallocated = 0;
  }
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    mu[ele_ind] += x[i][0]/N_elements_per_electron;
    mu[ele_ind+N_electrons] += x[i][1]/N_elements_per_electron;;
    mu[ele_ind+2*N_electrons] += x[i][2]/N_elements_per_electron;;
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    ele_ind = floor((tagid[i]-tag_ele_start)/N_elements_per_electron);
    x_COM[i] = mu_all[ele_ind];
    y_COM[i] = mu_all[ele_ind+N_electrons];
    z_COM[i] = mu_all[ele_ind+2*N_electrons];
    // fprintf(screen,"\nx[i][0] = %16.16f",x[i][0]);
...
int FixDynamicWidthsCoMNoPBC::pack_forward_comm(int n, int *list, double *buf,
...
void FixDynamicWidthsCoMNoPBC::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
...
int FixDynamicWidthsCoMNoPBC::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void FixDynamicWidthsCoMNoPBC::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
void FixDynamicWidthsCoMNoPBC::allocate()
{
  // fprintf(screen,"\nFixDynamicWidthsCoMNoPBC::allocate()\n");
...
void FixDynamicWidthsCoMNoPBC::deallocate()
{
  // fprintf(screen,"\nFixDynamicWidthsCoMNoPBC::deallocate()\n");
  delete [] mu;
  delete [] mu_all;
  delete [] tau;
  delete [] tau_all;
