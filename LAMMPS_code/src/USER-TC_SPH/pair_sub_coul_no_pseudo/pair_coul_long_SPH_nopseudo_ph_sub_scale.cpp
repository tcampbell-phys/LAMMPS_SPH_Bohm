...
#include "pair_coul_long_SPH_nopseudo_ph_sub_scale.h"
#include <mpi.h>
#include <cmath>
#include <cstring>
#include "group.h"
...
PairCoulLongNopseudoSPHphsubScale::PairCoulLongNopseudoSPHphsubScale(LAMMPS *lmp) : Pair(lmp)
{
...
PairCoulLongNopseudoSPHphsubScale::~PairCoulLongNopseudoSPHphsubScale()
{
  if (copymode) return;
...
void PairCoulLongNopseudoSPHphsubScale::compute(int eflag, int vflag)
{
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        if (itype != ion_species) {
          ...
          if (jtype == ion_species){
            // ion neighbour
            theta_coul_ei[i] += occupation_fact*scale[itype][jtype]*fact_i*q[j]*exp(-rsq/(2*h2_i))/(h_i);
          }
        }
        ...
        if (itype == ion_species) {
          ...
          if (jtype != ion_species){
            // electron neighbour
            if (newton_pair || j < nlocal) {
              theta_coul_ei[j] += occupation_fact*scale[itype][jtype]*theta_const*((qtmp*q[j])/(rho_SPH[j]*omega_SPH[j]))*(1/width_SPH[j])*exp(-rsq/(2*width_SPH[j]*width_SPH[j]));
            }
          }
        }
      }
      ...
      if (rsq < cut_coulsq) {
        ...
        if (jtype != ion_species){
          ...
          if (itype == ion_species){
            ...
            force_fact = occupation_fact*coul_prefact*(erf(r/(sqrt2*h_j))/(r*r*r) - (sqrt2/sqrt_pi)*(exp(-rsq*hm2_j/2)/(h_j*rsq)));
          ...
          if (itype != ion_species){
            ...
            if ( tagid[j] >= hi_lim_lev || tagid[j] < lo_lim_lev ){
              ...
              ecoul += occupation_fact*coul_prefact*erf(r/(sqrt2*eff_width))/r;
        ...
        if (jtype == ion_species){
          ...
          if (itype != ion_species){
            ...
            force_fact = occupation_fact*coul_prefact*(erf(r/(sqrt2*h_i))/(r*r*r) - (sqrt2/sqrt_pi)*(exp(-rsq*hm2_i/2)/(h_i*rsq)));
...
void PairCoulLongNopseudoSPHphsubScale::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairCoulLongNopseudoSPHphsubScale::settings(int narg, char **arg)
{
  ...
  fprintf(screen,"\n PairCoulLongNopseudoSPHphsubScale occupation_fact = %f",occupation_fact);
  fprintf(screen,"\n PairCoulLongNopseudoSPHphsubScale N_epe = %d",N_epe);
...
void PairCoulLongNopseudoSPHphsubScale::coeff(int narg, char **arg)
{
...
void PairCoulLongNopseudoSPHphsubScale::init_style()
{
  if (!atom->q_flag)
...
double PairCoulLongNopseudoSPHphsubScale::init_one(int i, int j)
{
  scale[j][i] = scale[i][j];
...
void PairCoulLongNopseudoSPHphsubScale::write_restart(FILE *fp)
{
...
void PairCoulLongNopseudoSPHphsubScale::read_restart(FILE *fp)
{
...
void PairCoulLongNopseudoSPHphsubScale::write_restart_settings(FILE *fp)
{
...
void PairCoulLongNopseudoSPHphsubScale::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
// double PairCoulLongNopseudoSPHphsubScale::single(int i, int j, int /*itype*/, int /*jtype*/,
//                             double rsq,
...
void *PairCoulLongNopseudoSPHphsubScale::extract(const char *str, int &dim)
{
...
int PairCoulLongNopseudoSPHphsubScale::pack_forward_comm(int n, int *list, double *buf,
...
void PairCoulLongNopseudoSPHphsubScale::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
...
int PairCoulLongNopseudoSPHphsubScale::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairCoulLongNopseudoSPHphsubScale::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
