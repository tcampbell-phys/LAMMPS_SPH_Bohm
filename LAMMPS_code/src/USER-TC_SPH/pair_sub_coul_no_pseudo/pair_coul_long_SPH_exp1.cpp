...
#include "pair_coul_long_SPH_exp1.h"
#include <mpi.h>
#include <cmath>
#include <cstring>
#include "group.h"
...
PairCoulLongNopseudoSPHexp::PairCoulLongNopseudoSPHexp(LAMMPS *lmp) : Pair(lmp)
{
...
PairCoulLongNopseudoSPHexp::~PairCoulLongNopseudoSPHexp()
{
  if (copymode) return;
...
void PairCoulLongNopseudoSPHexp::compute(int eflag, int vflag)
{
  ...
  if (atom->nmax > nmax) {
    ...
    memory->create(global_tag,nmax,"pair:global_tag");
  }
  ...
  // forward communicate the tagid to ghost processors

  for (ii = 0; ii < inum; ii++) {
    i = ilist[ii];
    global_tag[i] = tagid[i];
  }
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    if (itype != ion_species) {
      ...
      lo_lim_lev = floor((global_tag[i]-tag_ele_start)/N_epe)*N_epe + tag_ele_start;
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (itype != ion_species) {
        if ( global_tag[j] < hi_lim_lev ){
          if ( global_tag[j] >= lo_lim_lev ){
            continue;
          }
        }
      }
      ...
      if (rsq < cut_coulsq) {
        ...
        if (jtype != ion_species){
          ...
          if (itype != ion_species){
            ...
            if (global_tag[j] != tagid[j]){
              fprintf(screen,"\nglobal_tag[j] = %d",global_tag[j]);
              fprintf(screen,"\ntagid[j] = %d",tagid[j]);
            }

            if ( global_tag[j] >= hi_lim_lev || global_tag[j] < lo_lim_lev ){
...
void PairCoulLongNopseudoSPHexp::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairCoulLongNopseudoSPHexp::settings(int narg, char **arg)
{
  ...
  ele_TFWHM = force->numeric(FLERR,arg[4]);
  N_epe = force->numeric(FLERR,arg[5]);
  tag_ele_start = force->numeric(FLERR,arg[6]);
...
void PairCoulLongNopseudoSPHexp::coeff(int narg, char **arg)
{
...
void PairCoulLongNopseudoSPHexp::init_style()
{
  if (!atom->q_flag)
...
double PairCoulLongNopseudoSPHexp::init_one(int i, int j)
{
  scale[j][i] = scale[i][j];
...
void PairCoulLongNopseudoSPHexp::write_restart(FILE *fp)
{
...
void PairCoulLongNopseudoSPHexp::read_restart(FILE *fp)
{
...
void PairCoulLongNopseudoSPHexp::write_restart_settings(FILE *fp)
{
...
void PairCoulLongNopseudoSPHexp::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
// double PairCoulLongNopseudoSPHexp::single(int i, int j, int /*itype*/, int /*jtype*/,
//                             double rsq,
...
void *PairCoulLongNopseudoSPHexp::extract(const char *str, int &dim)
{
...
int PairCoulLongNopseudoSPHexp::pack_forward_comm(int n, int *list, double *buf,
...
void PairCoulLongNopseudoSPHexp::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
...
int PairCoulLongNopseudoSPHexp::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairCoulLongNopseudoSPHexp::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
