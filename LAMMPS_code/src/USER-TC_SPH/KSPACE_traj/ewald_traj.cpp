...
EwaldTraj::EwaldTraj(LAMMPS *lmp) : KSpace(lmp),
...
void EwaldTraj::settings(int narg, char **arg)
{
  if (narg != 3) error->all(FLERR,"Illegal kspace_style ewald command");

  accuracy_relative = fabs(force->numeric(FLERR,arg[0]));
  occupation_fact = force->numeric(FLERR,arg[1]);
  ele_type = force->numeric(FLERR,arg[2]);
  fprintf(screen,"\nEwaldTraj occupation_fact = %f",occupation_fact);
  fprintf(screen,"\nEwaldTraj ele_type = %d",ele_type);
...
{
  if (comm->me == 0) {
    if (screen) fprintf(screen,"EwaldTraj initialization ...\n");
    if (logfile) fprintf(logfile,"EwaldTraj initialization ...\n");
  }
  ...
  if (domain->dimension == 2)
    error->all(FLERR,"Cannot use EwaldTraj with 2d simulation");

  if (!atom->q_flag) error->all(FLERR,"Kspace style requires atom attribute q");

  if (slabflag == 0 && domain->nonperiodic > 0)
    error->all(FLERR,"Cannot use non-periodic boundaries with EwaldTraj");
  if (slabflag) {
    if (domain->xperiodic != 1 || domain->yperiodic != 1 ||
        domain->boundary[2][0] != 1 || domain->boundary[2][1] != 1)
      error->all(FLERR,"Incorrect boundaries with slab EwaldTraj");
    if (domain->triclinic)
      error->all(FLERR,"Cannot (yet) use EwaldTraj with triclinic box "
                 "and slab correction");
  }
  ...
  scale = 1.0;
  qqrd2e = force->qqrd2e;
  qsum_qsq_traj(1,occupation_fact,ele_type);
  ...
  // adjust z dimension for 2d slab EwaldTraj
  // 3d EwaldTraj just uses zprd since slab_volfactor = 1.0
  ...
  // setup EwaldTraj coefficients so can print stats
...
/* ----------------------------------------------------------------------
   adjust EwaldTraj coeffs, called initially and whenever volume has changed
------------------------------------------------------------------------- */

void EwaldTraj::setup()
{
  ...
  // adjustment of z dimension for 2d slab EwaldTraj
  // 3d EwaldTraj just uses zprd since slab_volfactor = 1.0
  ...
  // pre-compute EwaldTraj coefficients
...
double EwaldTraj::rms(int km, double prd, bigint natoms, double q2)
{
...
/* ----------------------------------------------------------------------
   compute the EwaldTraj long-range force, energy, virial
------------------------------------------------------------------------- */

void EwaldTraj::compute(int eflag, int vflag)
{
  int i,j,k;
  int *type = atom->type;
  double q_use;
  ...
  for (k = 0; k < kcount; k++) {
    ...
    for (i = 0; i < nlocal; i++) {
      ...
      if (evflag_atom) {
        ...
        if (eflag_atom) {
          if (type[i] == ele_type){
            q_use = occupation_fact*q[i];
          }
          if (type[i] != ele_type){
            q_use = q[i];
          }
          eatom[i] += q_use*ug[k]*partial_peratom;
        }
        if (vflag_atom)
          for (j = 0; j < 6; j++)
  ...
  if (evflag_atom) {
    if (eflag_atom) {
      for (i = 0; i < nlocal; i++) {
        ...
        eatom[i] -= g_ewald*q_use*q_use/MY_PIS + MY_PI2*q_use*qsum /
...
void EwaldTraj::eik_dot_r()
{
  ...
  // fprintf(screen,"\newald_scale q[0] = %f",q[0]);
  // fprintf(screen,"\newald_scale type[0] = %d",type[0]);
  // fprintf(screen,"\newald_scale q[10] = %f",q[10]);
  // fprintf(screen,"\newald_scale type[10] = %d",type[10]);
  // fprintf(screen,"\newald_scale q[20] = %f",q[20]);
  // fprintf(screen,"\newald_scale type[20] = %d",type[20]);
  // fprintf(screen,"\newald_scale q[300] = %f",q[300]);
  // fprintf(screen,"\newald_scale type[300] = %d",type[300]);
  // fprintf(screen,"\newald_scale q[400] = %f",q[400]);
  // fprintf(screen,"\newald_scale type[400] = %d",type[400]);
  // fprintf(screen,"\newald_scale q[1000] = %f",q[1000]);
  // fprintf(screen,"\newald_scale type[1000] = %d",type[1000]);
  ...
  for (ic = 0; ic < 3; ic++) {
    ...
    if (sqk <= gsqmx) {
      ...
      for (i = 0; i < nlocal; i++) {
        ...
        sn[-1][ic][i] = -sn[1][ic][i];
        if (type[i] == ele_type){
          cstr1 += occupation_fact*q[i]*cs[1][ic][i];
          sstr1 += occupation_fact*q[i]*sn[1][ic][i];
        }
        if (type[i] != ele_type){
          cstr1 += q[i]*cs[1][ic][i];
          sstr1 += q[i]*sn[1][ic][i];
    ...
    for (ic = 0; ic < 3; ic++) {
      ...
      if (sqk <= gsqmx) {
        ...
        for (i = 0; i < nlocal; i++) {
          ...
          sn[-m][ic][i] = -sn[m][ic][i];
          if (type[i] == ele_type){
            cstr1 += occupation_fact*q[i]*cs[m][ic][i];
            sstr1 += occupation_fact*q[i]*sn[m][ic][i];
          }
          if (type[i] != ele_type){
            cstr1 += q[i]*cs[m][ic][i];
            sstr1 += q[i]*sn[m][ic][i];
  ...
  for (k = 1; k <= kxmax; k++) {
    for (l = 1; l <= kymax; l++) {
      ...
      if (sqk <= gsqmx) {
        ...
        for (i = 0; i < nlocal; i++) {
          if (type[i] == ele_type){
            cstr1 += occupation_fact*q[i]*(cs[k][0][i]*cs[l][1][i] - sn[k][0][i]*sn[l][1][i]);
            sstr1 += occupation_fact*q[i]*(sn[k][0][i]*cs[l][1][i] + cs[k][0][i]*sn[l][1][i]);
            cstr2 += occupation_fact*q[i]*(cs[k][0][i]*cs[l][1][i] + sn[k][0][i]*sn[l][1][i]);
            sstr2 += occupation_fact*q[i]*(sn[k][0][i]*cs[l][1][i] - cs[k][0][i]*sn[l][1][i]);
          }
          if (type[i] != ele_type){
  ...
  for (l = 1; l <= kymax; l++) {
    for (m = 1; m <= kzmax; m++) {
      ...
      if (sqk <= gsqmx) {
        ...
        for (i = 0; i < nlocal; i++) {
          if (type[i] == ele_type){
            cstr1 += occupation_fact*q[i]*(cs[l][1][i]*cs[m][2][i] - sn[l][1][i]*sn[m][2][i]);
            sstr1 += occupation_fact*q[i]*(sn[l][1][i]*cs[m][2][i] + cs[l][1][i]*sn[m][2][i]);
            cstr2 += occupation_fact*q[i]*(cs[l][1][i]*cs[m][2][i] + sn[l][1][i]*sn[m][2][i]);
            sstr2 += occupation_fact*q[i]*(sn[l][1][i]*cs[m][2][i] - cs[l][1][i]*sn[m][2][i]);
          }
          if (type[i] != ele_type){
  ...
  for (k = 1; k <= kxmax; k++) {
    for (m = 1; m <= kzmax; m++) {
      ...
      if (sqk <= gsqmx) {
        ...
        for (i = 0; i < nlocal; i++) {
          if (type[i] == ele_type){
            cstr1 += occupation_fact*q[i]*(cs[k][0][i]*cs[m][2][i] - sn[k][0][i]*sn[m][2][i]);
            sstr1 += occupation_fact*q[i]*(sn[k][0][i]*cs[m][2][i] + cs[k][0][i]*sn[m][2][i]);
            cstr2 += occupation_fact*q[i]*(cs[k][0][i]*cs[m][2][i] + sn[k][0][i]*sn[m][2][i]);
            sstr2 += occupation_fact*q[i]*(sn[k][0][i]*cs[m][2][i] - cs[k][0][i]*sn[m][2][i]);
          }
          if (type[i] != ele_type){
      ...
      for (m = 1; m <= kzmax; m++) {
        ...
        if (sqk <= gsqmx) {
          ...
          for (i = 0; i < nlocal; i++) {
            ...
            if (type[i] == ele_type){
              cstr1 += occupation_fact*q[i]*(cs[k][0][i]*clpm - sn[k][0][i]*slpm);
              sstr1 += occupation_fact*q[i]*(sn[k][0][i]*clpm + cs[k][0][i]*slpm);
            }
            if (type[i] != ele_type){
            ...
            slpm = -sn[l][1][i]*cs[m][2][i] + cs[l][1][i]*sn[m][2][i];
            if (type[i] == ele_type){
              cstr2 += occupation_fact*q[i]*(cs[k][0][i]*clpm - sn[k][0][i]*slpm);
              sstr2 += occupation_fact*q[i]*(sn[k][0][i]*clpm + cs[k][0][i]*slpm);
            }
            if (type[i] != ele_type){
            ...
            slpm = sn[l][1][i]*cs[m][2][i] - cs[l][1][i]*sn[m][2][i];
            if (type[i] == ele_type){
              cstr3 += occupation_fact*q[i]*(cs[k][0][i]*clpm - sn[k][0][i]*slpm);
              sstr3 += occupation_fact*q[i]*(sn[k][0][i]*clpm + cs[k][0][i]*slpm);
            }
            if (type[i] != ele_type){
            ...
            slpm = -sn[l][1][i]*cs[m][2][i] - cs[l][1][i]*sn[m][2][i];
            if (type[i] == ele_type){
              cstr4 += occupation_fact*q[i]*(cs[k][0][i]*clpm - sn[k][0][i]*slpm);
              sstr4 += occupation_fact*q[i]*(sn[k][0][i]*clpm + cs[k][0][i]*slpm);
            }
            if (type[i] != ele_type){
...
void EwaldTraj::eik_dot_r_triclinic()
{
  int i,k,l,m,n,ic;
  double cstr1,sstr1;
  double sqk,clpm,slpm;
...
/* ----------------------------------------------------------------------
   pre-compute coefficients for each EwaldTraj K-vector
------------------------------------------------------------------------- */

void EwaldTraj::coeffs()
{
  int k,l,m;
  double sqk,vterm;
...
/* ----------------------------------------------------------------------
   pre-compute coefficients for each EwaldTraj K-vector for a triclinic
   system
------------------------------------------------------------------------- */

void EwaldTraj::coeffs_triclinic()
{
  int k,l,m;
  double sqk,vterm;
...
/* ----------------------------------------------------------------------
   ...
   periodically repeating slabs.  Yields good approximation to 2D EwaldTraj if
   adequate empty space is left between repeating slabs (J. Chem. Phys.
   111, 3155).  Slabs defined here to be parallel to the xy plane. Also
   extended to non-neutral systems (J. Chem. Phys. 131, 094107).
------------------------------------------------------------------------- */

void EwaldTraj::slabcorr()
{
...
/* ----------------------------------------------------------------------
   compute the EwaldTraj total long-range force and energy for groups A and B
 ------------------------------------------------------------------------- */

void EwaldTraj::compute_group_group(int groupbit_A, int groupbit_B, int AA_flag)
{
...
void EwaldTraj::slabcorr_groups(int groupbit_A, int groupbit_B, int AA_flag)
{
  // compute local contribution to global dipole moment
...
void EwaldTraj::allocate_groups()
{
  // group A
...
void EwaldTraj::deallocate_groups()
{
  // group A
