...
PairFilinovTriplet::PairFilinovTriplet(LAMMPS *lmp) : Pair(lmp)
{
...
PairFilinovTriplet::~PairFilinovTriplet()
{
  if (allocated) {
...
void PairFilinovTriplet::compute(int eflag, int vflag)
{
  ...
  double pot_pre_fact = - boltz_val * targ_temp;
  double for_pre_fact = (2 * boltz_val * targ_temp * boltz_val * targ_temp)/(hh_me);
  double exp_fact = (boltz_val * targ_temp)/(hh_me);
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        if (eflag) eXC = pot_pre_fact*log(1 - exp(-exp_fact*rsq));
...
void PairFilinovTriplet::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairFilinovTriplet::settings(int narg, char **arg)
{
...
void PairFilinovTriplet::coeff(int narg, char **arg)
{
...
void PairFilinovTriplet::init_style()
{
...
double PairFilinovTriplet::init_one(int i, int j)
{
  if (setflag[i][j] == 0)
...
void PairFilinovTriplet::write_restart(FILE *fp)
{
...
void PairFilinovTriplet::read_restart(FILE *fp)
{
...
void PairFilinovTriplet::write_restart_settings(FILE *fp)
{
...
void PairFilinovTriplet::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
// double PairFilinovTriplet::single(int i, int j, int /*itype*/, int /*jtype*/,
//                             double rsq,
//                             double targ_temp)
// {
...
void *PairFilinovTriplet::extract(const char *str, int &dim)
{
