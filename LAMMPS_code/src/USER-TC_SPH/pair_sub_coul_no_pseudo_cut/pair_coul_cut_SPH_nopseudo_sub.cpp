...
#include "pair_coul_cut_SPH_nopseudo_sub.h"
#include <mpi.h>
#include <cmath>
#include <cstring>
#include "atom.h"
...
PairCoulCutSPHNoPseudoSub::PairCoulCutSPHNoPseudoSub(LAMMPS *lmp) : Pair(lmp) {
  nmax = 0;
...
PairCoulCutSPHNoPseudoSub::~PairCoulCutSPHNoPseudoSub()
{
  if (allocated) {
...
void PairCoulCutSPHNoPseudoSub::compute(int eflag, int vflag)
{
...
void PairCoulCutSPHNoPseudoSub::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairCoulCutSPHNoPseudoSub::coeff(int narg, char **arg)
{
  if (narg < 2 || narg > 3)
...
void PairCoulCutSPHNoPseudoSub::init_style()
{
  if (!atom->q_flag)
...
double PairCoulCutSPHNoPseudoSub::init_one(int i, int j)
{
  if (setflag[i][j] == 0)
...
void PairCoulCutSPHNoPseudoSub::write_restart(FILE *fp)
{
...
void PairCoulCutSPHNoPseudoSub::read_restart(FILE *fp)
{
...
void PairCoulCutSPHNoPseudoSub::write_restart_settings(FILE *fp)
{
...
void PairCoulCutSPHNoPseudoSub::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
void *PairCoulCutSPHNoPseudoSub::extract(const char *str, int &dim)
{
  dim = 2;
...
int PairCoulCutSPHNoPseudoSub::pack_forward_comm(int n, int *list, double *buf,
...
void PairCoulCutSPHNoPseudoSub::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
...
int PairCoulCutSPHNoPseudoSub::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairCoulCutSPHNoPseudoSub::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
