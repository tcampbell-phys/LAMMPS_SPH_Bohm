...
#include "fix_dynamic_widths_CoM_BP_doub_rob.h"
#include <cstring>
#include <mpi.h>
#include "group.h"
#include "modify.h"
...
FixDynamicWidthsCoMBPDoubRob::FixDynamicWidthsCoMBPDoubRob(LAMMPS *lmp, int narg, char **arg):
	Fix(lmp, narg, arg), mu(NULL), mu_all(NULL), tau(NULL), tau_all(NULL)
{
  ...
  // // fprintf(screen,"\nIn FixDynamicWidthsCoMBPDoubRob with MPI_Allreduce and no commflag = 4 or 5");

  eflag = 0;
  eBohm = 0.0;
}

FixDynamicWidthsCoMBPDoubRob::~FixDynamicWidthsCoMBPDoubRob()
{
...
int FixDynamicWidthsCoMBPDoubRob::setmask()
{
  int mask = 0;
...
void FixDynamicWidthsCoMBPDoubRob::init()
{ 
...
void FixDynamicWidthsCoMBPDoubRob::setup_pre_force(int)
{
  FixedPointIterator();
  CoM_Calculator();
}

void FixDynamicWidthsCoMBPDoubRob::min_pre_force(int)
{
  FixedPointIterator();
  CoM_Calculator();
}
...
void FixDynamicWidthsCoMBPDoubRob::setup_post_neighbor()
{
...
void FixDynamicWidthsCoMBPDoubRob::post_neighbor()
{
  return;
}

void FixDynamicWidthsCoMBPDoubRob::pre_force(int)
{
  FixedPointIterator();
  CoM_Calculator();
}

void FixDynamicWidthsCoMBPDoubRob::end_of_step()
{
  // fprintf(screen,"\nFixDynamicWidthsCoMBPDoubRob end_of_step");
  CalcBohmPot();
}

void FixDynamicWidthsCoMBPDoubRob::FixedPointIterator()
{
...
double FixDynamicWidthsCoMBPDoubRob::Gauss_Width_Deriv(double pre_fact, double wid, double sep_sq)
{
  return pre_fact*exp(-sep_sq/(wid*wid*2.0))*((sep_sq)/(wid*wid*wid) - 3.0/wid);
}

void FixDynamicWidthsCoMBPDoubRob::CoM_Calculator()
{
  int i,ii,inum;
  int *ilist;
	double **x = atom->x;
  ...
  if (unallocated){
    deallocate();
    // // // fprintf(screen,"\nFixDynamicWidthsCoMBPDoubRob allocating mu tau memory...\n");
    allocate();
    allocated = 1;
    unallocated = 0;
  }
...
void FixDynamicWidthsCoMBPDoubRob::CalcBohmPot()
{
  // fprintf(screen,"\nCheckpoint 0");
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsquared) {
        ...
        dxx_rho_fix[i] += dx_rho_fix[j]*((-delx)*hm2_i)*m_gauss_ij_rho;
        dxy_rho_fix[i] += dy_rho_fix[j]*((-delx)*hm2_i)*m_gauss_ij_rho;
        dxz_rho_fix[i] += dz_rho_fix[j]*((-delx)*hm2_i)*m_gauss_ij_rho;
        dyy_rho_fix[i] += dy_rho_fix[j]*((-dely)*hm2_i)*m_gauss_ij_rho;
        dyx_rho_fix[i] += dx_rho_fix[j]*((-dely)*hm2_i)*m_gauss_ij_rho;
        dyz_rho_fix[i] += dz_rho_fix[j]*((-dely)*hm2_i)*m_gauss_ij_rho;
        dzx_rho_fix[i] += dx_rho_fix[j]*((-delz)*hm2_i)*m_gauss_ij_rho;
        dzy_rho_fix[i] += dy_rho_fix[j]*((-delz)*hm2_i)*m_gauss_ij_rho;
        dzz_rho_fix[i] += dz_rho_fix[j]*((-delz)*hm2_i)*m_gauss_ij_rho;
        ...
        if (newton_pair || j < nlocal) {
          ...
          dxx_rho_fix[j] -= dx_rho_fix[i]*((-delx)*hm2_j)*m_gauss_ji_rho;
          dxy_rho_fix[j] -= dy_rho_fix[i]*((-delx)*hm2_j)*m_gauss_ji_rho;
          dxz_rho_fix[j] -= dz_rho_fix[i]*((-delx)*hm2_j)*m_gauss_ji_rho;
          dyy_rho_fix[j] -= dy_rho_fix[i]*((-dely)*hm2_j)*m_gauss_ji_rho;
          dyx_rho_fix[j] -= dx_rho_fix[i]*((-dely)*hm2_j)*m_gauss_ji_rho;
          dyz_rho_fix[j] -= dz_rho_fix[i]*((-dely)*hm2_j)*m_gauss_ji_rho;
          dzx_rho_fix[j] -= dx_rho_fix[i]*((-delz)*hm2_j)*m_gauss_ji_rho;
          dzy_rho_fix[j] -= dy_rho_fix[i]*((-delz)*hm2_j)*m_gauss_ji_rho;
          dzz_rho_fix[j] -= dz_rho_fix[i]*((-delz)*hm2_j)*m_gauss_ji_rho;
        }
      }
    }
  }
...
int FixDynamicWidthsCoMBPDoubRob::pack_forward_comm(int n, int *list, double *buf,
...
void FixDynamicWidthsCoMBPDoubRob::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
...
int FixDynamicWidthsCoMBPDoubRob::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void FixDynamicWidthsCoMBPDoubRob::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
void FixDynamicWidthsCoMBPDoubRob::allocate()
{
  // // fprintf(screen,"\nFixDynamicWidthsCoMBPDoubRob::allocate()\n");
...
void FixDynamicWidthsCoMBPDoubRob::deallocate()
{
  // // fprintf(screen,"\nFixDynamicWidthsCoMBPDoubRob::deallocate()\n");
  delete [] mu;
  delete [] mu_all;
  delete [] tau;
  delete [] tau_all;
...
double FixDynamicWidthsCoMBPDoubRob::compute_scalar()
{
