...
#include "fix_dynamic_widths_CoM_BP_Mocz_ftwo_diag.h"
#include <cstring>
#include <mpi.h>
#include "group.h"
#include "modify.h"
...
FixDynamicWidthsCoMBPMoczFtwoDiag::FixDynamicWidthsCoMBPMoczFtwoDiag(LAMMPS *lmp, int narg, char **arg):
	Fix(lmp, narg, arg), mu(NULL), mu_all(NULL), tau(NULL), tau_all(NULL)
{
  ...
  // // fprintf(screen,"\nIn FixDynamicWidthsCoMBPMoczFtwoDiag with MPI_Allreduce and no commflag = 4 or 5");

  eflag = 0;
  eBohm = 0.0;
}

FixDynamicWidthsCoMBPMoczFtwoDiag::~FixDynamicWidthsCoMBPMoczFtwoDiag()
{
...
int FixDynamicWidthsCoMBPMoczFtwoDiag::setmask()
{
  int mask = 0;
...
void FixDynamicWidthsCoMBPMoczFtwoDiag::init()
{ 
...
void FixDynamicWidthsCoMBPMoczFtwoDiag::setup_pre_force(int)
{
  FixedPointIterator();
  CoM_Calculator();
}

void FixDynamicWidthsCoMBPMoczFtwoDiag::min_pre_force(int)
{
  FixedPointIterator();
  CoM_Calculator();
}
...
void FixDynamicWidthsCoMBPMoczFtwoDiag::setup_post_neighbor()
{
...
void FixDynamicWidthsCoMBPMoczFtwoDiag::post_neighbor()
{
  return;
}

void FixDynamicWidthsCoMBPMoczFtwoDiag::pre_force(int)
{
  FixedPointIterator();
  CoM_Calculator();
}

void FixDynamicWidthsCoMBPMoczFtwoDiag::end_of_step()
{
  // fprintf(screen,"\nFixDynamicWidthsCoMBPMoczFtwoDiag end_of_step");
  CalcBohmPot();
}

void FixDynamicWidthsCoMBPMoczFtwoDiag::FixedPointIterator()
{
...
double FixDynamicWidthsCoMBPMoczFtwoDiag::Gauss_Width_Deriv(double pre_fact, double wid, double sep_sq)
{
  return pre_fact*exp(-sep_sq/(wid*wid*2.0))*((sep_sq)/(wid*wid*wid) - 3.0/wid);
}

void FixDynamicWidthsCoMBPMoczFtwoDiag::CoM_Calculator()
{
  int i,ii,inum;
  int *ilist;
	double **x = atom->x;
  ...
  if (unallocated){
    deallocate();
    // // // fprintf(screen,"\nFixDynamicWidthsCoMBPMoczFtwoDiag allocating mu tau memory...\n");
    allocate();
    allocated = 1;
    unallocated = 0;
  }
...
void FixDynamicWidthsCoMBPMoczFtwoDiag::CalcBohmPot()
{
  // fprintf(screen,"\nCheckpoint 0");
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    Pxx_fix[i] += gamma_factor*f_prefactor*((dx_rho_fix[i]*dx_rho_fix[i])/rho_SPH[i] - dxx_rho_fix[i]);
    Pyy_fix[i] += gamma_factor*f_prefactor*((dy_rho_fix[i]*dy_rho_fix[i])/rho_SPH[i] - dyy_rho_fix[i]);
    Pzz_fix[i] += gamma_factor*f_prefactor*((dz_rho_fix[i]*dz_rho_fix[i])/rho_SPH[i] - dzz_rho_fix[i]);
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    // if (tag[i] == tag_targ) {
    //   fprintf(screen,"\nfix_dynamic_widths_CoM_BP_Mocz_rob_ftwo tag_targ = %d",tag_targ);
...
int FixDynamicWidthsCoMBPMoczFtwoDiag::pack_forward_comm(int n, int *list, double *buf,
...
void FixDynamicWidthsCoMBPMoczFtwoDiag::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
...
int FixDynamicWidthsCoMBPMoczFtwoDiag::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void FixDynamicWidthsCoMBPMoczFtwoDiag::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
void FixDynamicWidthsCoMBPMoczFtwoDiag::allocate()
{
  // // fprintf(screen,"\nFixDynamicWidthsCoMBPMoczFtwoDiag::allocate()\n");
...
void FixDynamicWidthsCoMBPMoczFtwoDiag::deallocate()
{
  // // fprintf(screen,"\nFixDynamicWidthsCoMBPMoczFtwoDiag::deallocate()\n");
  delete [] mu;
  delete [] mu_all;
  delete [] tau;
  delete [] tau_all;
...
double FixDynamicWidthsCoMBPMoczFtwoDiag::compute_scalar()
{
