...
#include "fix_dynamic_widths_CoM_BP_Mocz_rob.h"
#include <cstring>
#include <mpi.h>
#include "group.h"
#include "modify.h"
...
FixDynamicWidthsCoMBPMoczRob::FixDynamicWidthsCoMBPMoczRob(LAMMPS *lmp, int narg, char **arg):
	Fix(lmp, narg, arg), mu(NULL), mu_all(NULL), tau(NULL), tau_all(NULL)
{
  ...
  // // fprintf(screen,"\nIn FixDynamicWidthsCoMBPMoczRob with MPI_Allreduce and no commflag = 4 or 5");

  eflag = 0;
  eBohm = 0.0;
}

FixDynamicWidthsCoMBPMoczRob::~FixDynamicWidthsCoMBPMoczRob()
{
...
int FixDynamicWidthsCoMBPMoczRob::setmask()
{
  int mask = 0;
...
void FixDynamicWidthsCoMBPMoczRob::init()
{ 
...
void FixDynamicWidthsCoMBPMoczRob::setup_pre_force(int)
{
  FixedPointIterator();
  CoM_Calculator();
}

void FixDynamicWidthsCoMBPMoczRob::min_pre_force(int)
{
  FixedPointIterator();
  CoM_Calculator();
}
...
void FixDynamicWidthsCoMBPMoczRob::setup_post_neighbor()
{
...
void FixDynamicWidthsCoMBPMoczRob::post_neighbor()
{
  return;
}

void FixDynamicWidthsCoMBPMoczRob::pre_force(int)
{
  FixedPointIterator();
  CoM_Calculator();
}

void FixDynamicWidthsCoMBPMoczRob::end_of_step()
{
  // fprintf(screen,"\nFixDynamicWidthsCoMBPMoczRob end_of_step");
  CalcBohmPot();
}

void FixDynamicWidthsCoMBPMoczRob::FixedPointIterator()
{
...
double FixDynamicWidthsCoMBPMoczRob::Gauss_Width_Deriv(double pre_fact, double wid, double sep_sq)
{
  return pre_fact*exp(-sep_sq/(wid*wid*2.0))*((sep_sq)/(wid*wid*wid) - 3.0/wid);
}

void FixDynamicWidthsCoMBPMoczRob::CoM_Calculator()
{
  int i,ii,inum;
  int *ilist;
	double **x = atom->x;
  ...
  if (unallocated){
    deallocate();
    // // // fprintf(screen,"\nFixDynamicWidthsCoMBPMoczRob allocating mu tau memory...\n");
    allocate();
    allocated = 1;
    unallocated = 0;
  }
...
void FixDynamicWidthsCoMBPMoczRob::CalcBohmPot()
{
  // fprintf(screen,"\nCheckpoint 0");
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    // if (tag[i] == tag_targ) {
    //   fprintf(screen,"\nfix_dynamic_widths_CoM_BP_Mocz_rob tag_targ = %d",tag_targ);
...
int FixDynamicWidthsCoMBPMoczRob::pack_forward_comm(int n, int *list, double *buf,
...
void FixDynamicWidthsCoMBPMoczRob::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
...
int FixDynamicWidthsCoMBPMoczRob::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void FixDynamicWidthsCoMBPMoczRob::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
void FixDynamicWidthsCoMBPMoczRob::allocate()
{
  // // fprintf(screen,"\nFixDynamicWidthsCoMBPMoczRob::allocate()\n");
...
void FixDynamicWidthsCoMBPMoczRob::deallocate()
{
  // // fprintf(screen,"\nFixDynamicWidthsCoMBPMoczRob::deallocate()\n");
  delete [] mu;
  delete [] mu_all;
  delete [] tau;
  delete [] tau_all;
...
double FixDynamicWidthsCoMBPMoczRob::compute_scalar()
{
