...
#include "fix_dynamic_widths_CoM_BP_Mocz_rob_ftwo_diag.h"
#include <cstring>
#include <mpi.h>
#include "group.h"
#include "modify.h"
...
FixDynamicWidthsCoMBPMoczRobFtwoDiag::FixDynamicWidthsCoMBPMoczRobFtwoDiag(LAMMPS *lmp, int narg, char **arg):
	Fix(lmp, narg, arg), mu(NULL), mu_all(NULL), tau(NULL), tau_all(NULL)
{
  ...
  // // fprintf(screen,"\nIn FixDynamicWidthsCoMBPMoczRobFtwoDiag with MPI_Allreduce and no commflag = 4 or 5");

  eflag = 0;
  eBohm = 0.0;
}

FixDynamicWidthsCoMBPMoczRobFtwoDiag::~FixDynamicWidthsCoMBPMoczRobFtwoDiag()
{
...
int FixDynamicWidthsCoMBPMoczRobFtwoDiag::setmask()
{
  int mask = 0;
...
void FixDynamicWidthsCoMBPMoczRobFtwoDiag::init()
{ 
...
void FixDynamicWidthsCoMBPMoczRobFtwoDiag::setup_pre_force(int)
{
  FixedPointIterator();
  CoM_Calculator();
}

void FixDynamicWidthsCoMBPMoczRobFtwoDiag::min_pre_force(int)
{
  FixedPointIterator();
  CoM_Calculator();
}
...
void FixDynamicWidthsCoMBPMoczRobFtwoDiag::setup_post_neighbor()
{
...
void FixDynamicWidthsCoMBPMoczRobFtwoDiag::post_neighbor()
{
  return;
}

void FixDynamicWidthsCoMBPMoczRobFtwoDiag::pre_force(int)
{
  FixedPointIterator();
  CoM_Calculator();
}

void FixDynamicWidthsCoMBPMoczRobFtwoDiag::end_of_step()
{
  // fprintf(screen,"\nFixDynamicWidthsCoMBPMoczRobFtwoDiag end_of_step");
  CalcBohmPot();
}

void FixDynamicWidthsCoMBPMoczRobFtwoDiag::FixedPointIterator()
{
...
double FixDynamicWidthsCoMBPMoczRobFtwoDiag::Gauss_Width_Deriv(double pre_fact, double wid, double sep_sq)
{
  return pre_fact*exp(-sep_sq/(wid*wid*2.0))*((sep_sq)/(wid*wid*wid) - 3.0/wid);
}

void FixDynamicWidthsCoMBPMoczRobFtwoDiag::CoM_Calculator()
{
  int i,ii,inum;
  int *ilist;
	double **x = atom->x;
  ...
  if (unallocated){
    deallocate();
    // // // fprintf(screen,"\nFixDynamicWidthsCoMBPMoczRobFtwoDiag allocating mu tau memory...\n");
    allocate();
    allocated = 1;
    unallocated = 0;
  }
...
void FixDynamicWidthsCoMBPMoczRobFtwoDiag::CalcBohmPot()
{
  // fprintf(screen,"\nCheckpoint 0");
...
int FixDynamicWidthsCoMBPMoczRobFtwoDiag::pack_forward_comm(int n, int *list, double *buf,
...
void FixDynamicWidthsCoMBPMoczRobFtwoDiag::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
...
int FixDynamicWidthsCoMBPMoczRobFtwoDiag::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void FixDynamicWidthsCoMBPMoczRobFtwoDiag::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
void FixDynamicWidthsCoMBPMoczRobFtwoDiag::allocate()
{
  // // fprintf(screen,"\nFixDynamicWidthsCoMBPMoczRobFtwoDiag::allocate()\n");
...
void FixDynamicWidthsCoMBPMoczRobFtwoDiag::deallocate()
{
  // // fprintf(screen,"\nFixDynamicWidthsCoMBPMoczRobFtwoDiag::deallocate()\n");
  delete [] mu;
  delete [] mu_all;
  delete [] tau;
  delete [] tau_all;
...
double FixDynamicWidthsCoMBPMoczRobFtwoDiag::compute_scalar()
{
