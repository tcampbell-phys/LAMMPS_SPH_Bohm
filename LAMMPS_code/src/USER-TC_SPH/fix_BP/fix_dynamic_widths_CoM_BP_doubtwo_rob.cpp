...
#include "fix_dynamic_widths_CoM_BP_doubtwo_rob.h"
#include <cstring>
#include <mpi.h>
#include "group.h"
#include "modify.h"
...
FixDynamicWidthsCoMBPDoubtwoRob::FixDynamicWidthsCoMBPDoubtwoRob(LAMMPS *lmp, int narg, char **arg):
	Fix(lmp, narg, arg), mu(NULL), mu_all(NULL), tau(NULL), tau_all(NULL)
{
  ...
  // // fprintf(screen,"\nIn FixDynamicWidthsCoMBPDoubtwoRob with MPI_Allreduce and no commflag = 4 or 5");

  eflag = 0;
  eBohm = 0.0;
}

FixDynamicWidthsCoMBPDoubtwoRob::~FixDynamicWidthsCoMBPDoubtwoRob()
{
  ...
  if (allocated){
    ...
    memory->destroy(dyx_rho_fix);
    memory->destroy(dzx_rho_fix);
    memory->destroy(dzy_rho_fix);
    ...
    memory->destroy(Pyx_fix);
    memory->destroy(Pzx_fix);
    memory->destroy(Pzy_fix);
  }
}

int FixDynamicWidthsCoMBPDoubtwoRob::setmask()
{
  int mask = 0;
...
void FixDynamicWidthsCoMBPDoubtwoRob::init()
{ 
...
void FixDynamicWidthsCoMBPDoubtwoRob::setup_pre_force(int)
{
  FixedPointIterator();
  CoM_Calculator();
}

void FixDynamicWidthsCoMBPDoubtwoRob::min_pre_force(int)
{
  FixedPointIterator();
  CoM_Calculator();
}
...
void FixDynamicWidthsCoMBPDoubtwoRob::setup_post_neighbor()
{
...
void FixDynamicWidthsCoMBPDoubtwoRob::post_neighbor()
{
  return;
}

void FixDynamicWidthsCoMBPDoubtwoRob::pre_force(int)
{
  FixedPointIterator();
  CoM_Calculator();
}

void FixDynamicWidthsCoMBPDoubtwoRob::end_of_step()
{
  // fprintf(screen,"\nFixDynamicWidthsCoMBPDoubtwoRob end_of_step");
  CalcBohmPot();
}

void FixDynamicWidthsCoMBPDoubtwoRob::FixedPointIterator()
{
...
double FixDynamicWidthsCoMBPDoubtwoRob::Gauss_Width_Deriv(double pre_fact, double wid, double sep_sq)
{
  return pre_fact*exp(-sep_sq/(wid*wid*2.0))*((sep_sq)/(wid*wid*wid) - 3.0/wid);
}

void FixDynamicWidthsCoMBPDoubtwoRob::CoM_Calculator()
{
  int i,ii,inum;
  int *ilist;
	double **x = atom->x;
  ...
  if (unallocated){
    deallocate();
    // // // fprintf(screen,"\nFixDynamicWidthsCoMBPDoubtwoRob allocating mu tau memory...\n");
    allocate();
    allocated = 1;
    unallocated = 0;
  }
...
void FixDynamicWidthsCoMBPDoubtwoRob::CalcBohmPot()
{
  // fprintf(screen,"\nCheckpoint 0");
  ...
  double m_gauss_ij_rho,m_gauss_ji_rho;
  double rho_i,rho_j;
  double bohm_pot;
  ...
  if (atom->nmax > nmax) {
    ...
    memory->create(dyx_rho_fix,nmax,"fix:dyx_rho_fix");
    memory->create(dzx_rho_fix,nmax,"fix:dzx_rho_fix");
    memory->create(dzy_rho_fix,nmax,"fix:dzy_rho_fix");
    ...
    memory->create(Pyx_fix,nmax,"fix:Pyx_fix");
    memory->create(Pzx_fix,nmax,"fix:Pzx_fix");
    memory->create(Pzy_fix,nmax,"fix:Pzy_fix");
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsquared) {
        ...
        dx_rho_fix[i] += ((-delx)*hm2_i)*m_gauss_ij;
        dy_rho_fix[i] += ((-dely)*hm2_i)*m_gauss_ij;
        dz_rho_fix[i] += ((-delz)*hm2_i)*m_gauss_ij;
        ...
        if (newton_pair || j < nlocal) {
          ...
          dx_rho_fix[j] += ((delx)*hm2_j)*m_gauss_ji;
          dy_rho_fix[j] += ((dely)*hm2_j)*m_gauss_ji;
          dz_rho_fix[j] += ((delz)*hm2_j)*m_gauss_ji;
        }
      }
    }
  }
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsquared) {
        ...
        m_gauss_ij_rho = jmass*gauss_pre_i*exp(-(rsq)*hm2_i/2.)/rho_j;

        dxx_rho_fix[i] += (dx_rho_fix[j]-dx_rho_fix[i])*((-delx)*hm2_i)*m_gauss_ij_rho;
        dxy_rho_fix[i] += (dy_rho_fix[j]-dy_rho_fix[i])*((-delx)*hm2_i)*m_gauss_ij_rho;
        dxz_rho_fix[i] += (dz_rho_fix[j]-dz_rho_fix[i])*((-delx)*hm2_i)*m_gauss_ij_rho;
        dyy_rho_fix[i] += (dy_rho_fix[j]-dy_rho_fix[i])*((-dely)*hm2_i)*m_gauss_ij_rho;
        dyx_rho_fix[i] += (dx_rho_fix[j]-dx_rho_fix[i])*((-dely)*hm2_i)*m_gauss_ij_rho;
        dyz_rho_fix[i] += (dz_rho_fix[j]-dz_rho_fix[i])*((-dely)*hm2_i)*m_gauss_ij_rho;
        dzx_rho_fix[i] += (dx_rho_fix[j]-dx_rho_fix[i])*((-delz)*hm2_i)*m_gauss_ij_rho;
        dzy_rho_fix[i] += (dy_rho_fix[j]-dy_rho_fix[i])*((-delz)*hm2_i)*m_gauss_ij_rho;
        dzz_rho_fix[i] += (dz_rho_fix[j]-dz_rho_fix[i])*((-delz)*hm2_i)*m_gauss_ij_rho;
        ...
        if (newton_pair || j < nlocal) {
          ...
          m_gauss_ji_rho = imass*gauss_pre_j*exp(-(rsq)*hm2_j/2.)/rho_i;

          dxx_rho_fix[j] -= (dx_rho_fix[i]-dx_rho_fix[j])*((-delx)*hm2_j)*m_gauss_ji_rho;
          dxy_rho_fix[j] -= (dy_rho_fix[i]-dy_rho_fix[j])*((-delx)*hm2_j)*m_gauss_ji_rho;
          dxz_rho_fix[j] -= (dz_rho_fix[i]-dz_rho_fix[j])*((-delx)*hm2_j)*m_gauss_ji_rho;
          dyy_rho_fix[j] -= (dy_rho_fix[i]-dy_rho_fix[j])*((-dely)*hm2_j)*m_gauss_ji_rho;
          dyx_rho_fix[j] -= (dx_rho_fix[i]-dx_rho_fix[j])*((-dely)*hm2_j)*m_gauss_ji_rho;
          dyz_rho_fix[j] -= (dz_rho_fix[i]-dz_rho_fix[j])*((-dely)*hm2_j)*m_gauss_ji_rho;
          dzx_rho_fix[j] -= (dx_rho_fix[i]-dx_rho_fix[j])*((-delz)*hm2_j)*m_gauss_ji_rho;
          dzy_rho_fix[j] -= (dy_rho_fix[i]-dy_rho_fix[j])*((-delz)*hm2_j)*m_gauss_ji_rho;
          dzz_rho_fix[j] -= (dz_rho_fix[i]-dz_rho_fix[j])*((-delz)*hm2_j)*m_gauss_ji_rho;
        }
      }
    }
  }
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    Pyx_fix[i] += SPH_weight_fact_self*gamma_factor*f_prefactor*((dy_rho_fix[i]*dx_rho_fix[i])/rho_SPH[i] - dyx_rho_fix[i]);
    Pzx_fix[i] += SPH_weight_fact_self*gamma_factor*f_prefactor*((dz_rho_fix[i]*dx_rho_fix[i])/rho_SPH[i] - dzx_rho_fix[i]);
    Pzy_fix[i] += SPH_weight_fact_self*gamma_factor*f_prefactor*((dz_rho_fix[i]*dy_rho_fix[i])/rho_SPH[i] - dzy_rho_fix[i]);
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsquared) {
        ...
        Pyx_fix[i] += exp_ij*SPH_weight_fact_ij*gamma_factor*f_prefactor*((dy_rho_fix[j]*dx_rho_fix[j])/rho_SPH[j] - dyx_rho_fix[j]);
        Pzx_fix[i] += exp_ij*SPH_weight_fact_ij*gamma_factor*f_prefactor*((dz_rho_fix[j]*dx_rho_fix[j])/rho_SPH[j] - dzx_rho_fix[j]);
        Pzy_fix[i] += exp_ij*SPH_weight_fact_ij*gamma_factor*f_prefactor*((dz_rho_fix[j]*dy_rho_fix[j])/rho_SPH[j] - dzy_rho_fix[j]);
        ...
        if (newton_pair || j < nlocal) {
          ...
          Pyx_fix[j] += exp_ji*SPH_weight_fact_ji*gamma_factor*f_prefactor*((dy_rho_fix[i]*dx_rho_fix[i])/rho_SPH[i] - dyx_rho_fix[i]);
          Pzx_fix[j] += exp_ji*SPH_weight_fact_ji*gamma_factor*f_prefactor*((dz_rho_fix[i]*dx_rho_fix[i])/rho_SPH[i] - dzx_rho_fix[i]);
          Pzy_fix[j] += exp_ji*SPH_weight_fact_ji*gamma_factor*f_prefactor*((dz_rho_fix[i]*dy_rho_fix[i])/rho_SPH[i] - dzy_rho_fix[i]);
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    // if (tag[i] == tag_targ) {
    //   fprintf(screen,"\nfix_dynamic_widths_CoM_BP_doubtwo_rob_ftwo tag_targ = %d",tag_targ);
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsquared) {
        ...
        u_SPH[i] += u_prefact_i*ijmass*((Pxx_fix[i]*delvx + Pxy_fix[i]*delvy + Pxz_fix[i]*delvz)*dx_Wij + (Pyx_fix[i]*delvx + Pyy_fix[i]*delvy + Pyz_fix[i]*delvz)*dy_Wij + (Pzx_fix[i]*delvx + Pzy_fix[i]*delvy + Pzz_fix[i]*delvz)*dz_Wij);

        if (newton_pair || j < nlocal) {

          u_SPH[j] -= u_prefact_j*ijmass*((-Pxx_fix[j]*delvx - Pxy_fix[j]*delvy - Pxz_fix[j]*delvz)*dx_Wji + (-Pyx_fix[j]*delvx - Pyy_fix[j]*delvy - Pyz_fix[j]*delvz)*dy_Wji + (-Pzx_fix[j]*delvx - Pzy_fix[j]*delvy - Pzz_fix[j]*delvz)*dz_Wji);
        }
      }
    }
  }
...
int FixDynamicWidthsCoMBPDoubtwoRob::pack_forward_comm(int n, int *list, double *buf,
...
void FixDynamicWidthsCoMBPDoubtwoRob::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
...
int FixDynamicWidthsCoMBPDoubtwoRob::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void FixDynamicWidthsCoMBPDoubtwoRob::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
void FixDynamicWidthsCoMBPDoubtwoRob::allocate()
{
  // // fprintf(screen,"\nFixDynamicWidthsCoMBPDoubtwoRob::allocate()\n");
...
void FixDynamicWidthsCoMBPDoubtwoRob::deallocate()
{
  // // fprintf(screen,"\nFixDynamicWidthsCoMBPDoubtwoRob::deallocate()\n");
  delete [] mu;
  delete [] mu_all;
  delete [] tau;
  delete [] tau_all;
...
double FixDynamicWidthsCoMBPDoubtwoRob::compute_scalar()
{
