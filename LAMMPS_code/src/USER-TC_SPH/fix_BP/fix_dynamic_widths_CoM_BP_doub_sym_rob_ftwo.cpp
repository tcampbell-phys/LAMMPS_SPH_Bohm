...
#include "fix_dynamic_widths_CoM_BP_doub_sym_rob_ftwo.h"
#include <cstring>
#include <mpi.h>
#include "group.h"
#include "modify.h"
...
FixDynamicWidthsCoMBPDoubSymRobFtwo::FixDynamicWidthsCoMBPDoubSymRobFtwo(LAMMPS *lmp, int narg, char **arg):
	Fix(lmp, narg, arg), mu(NULL), mu_all(NULL), tau(NULL), tau_all(NULL)
{
  ...
  // // fprintf(screen,"\nIn FixDynamicWidthsCoMBPDoubSymRobFtwo with MPI_Allreduce and no commflag = 4 or 5");

  eflag = 0;
  eBohm = 0.0;
}

FixDynamicWidthsCoMBPDoubSymRobFtwo::~FixDynamicWidthsCoMBPDoubSymRobFtwo()
{
...
int FixDynamicWidthsCoMBPDoubSymRobFtwo::setmask()
{
  int mask = 0;
...
void FixDynamicWidthsCoMBPDoubSymRobFtwo::init()
{ 
...
void FixDynamicWidthsCoMBPDoubSymRobFtwo::setup_pre_force(int)
{
  FixedPointIterator();
  CoM_Calculator();
}

void FixDynamicWidthsCoMBPDoubSymRobFtwo::min_pre_force(int)
{
  FixedPointIterator();
  CoM_Calculator();
}
...
void FixDynamicWidthsCoMBPDoubSymRobFtwo::setup_post_neighbor()
{
...
void FixDynamicWidthsCoMBPDoubSymRobFtwo::post_neighbor()
{
  return;
}

void FixDynamicWidthsCoMBPDoubSymRobFtwo::pre_force(int)
{
  FixedPointIterator();
  CoM_Calculator();
}

void FixDynamicWidthsCoMBPDoubSymRobFtwo::end_of_step()
{
  // fprintf(screen,"\nFixDynamicWidthsCoMBPDoubSymRobFtwo end_of_step");
  CalcBohmPot();
}

void FixDynamicWidthsCoMBPDoubSymRobFtwo::FixedPointIterator()
{
...
double FixDynamicWidthsCoMBPDoubSymRobFtwo::Gauss_Width_Deriv(double pre_fact, double wid, double sep_sq)
{
  return pre_fact*exp(-sep_sq/(wid*wid*2.0))*((sep_sq)/(wid*wid*wid) - 3.0/wid);
}

void FixDynamicWidthsCoMBPDoubSymRobFtwo::CoM_Calculator()
{
  int i,ii,inum;
  int *ilist;
	double **x = atom->x;
  ...
  if (unallocated){
    deallocate();
    // // // fprintf(screen,"\nFixDynamicWidthsCoMBPDoubSymRobFtwo allocating mu tau memory...\n");
    allocate();
    allocated = 1;
    unallocated = 0;
  }
...
void FixDynamicWidthsCoMBPDoubSymRobFtwo::CalcBohmPot()
{
  // fprintf(screen,"\nCheckpoint 0");
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    Pxy_fix[i] += SPH_weight_fact_self*gamma_factor*f_prefactor*((dx_rho_fix[i]*dy_rho_fix[i])/rho_SPH[i] - 0.5*(dxy_rho_fix[i]+dyx_rho_fix[i]));
    Pxz_fix[i] += SPH_weight_fact_self*gamma_factor*f_prefactor*((dx_rho_fix[i]*dz_rho_fix[i])/rho_SPH[i] - 0.5*(dxz_rho_fix[i]+dzx_rho_fix[i]));
    Pyy_fix[i] += SPH_weight_fact_self*gamma_factor*f_prefactor*((dy_rho_fix[i]*dy_rho_fix[i])/rho_SPH[i] - dyy_rho_fix[i]);
    Pyz_fix[i] += SPH_weight_fact_self*gamma_factor*f_prefactor*((dy_rho_fix[i]*dz_rho_fix[i])/rho_SPH[i] - 0.5*(dyz_rho_fix[i]+dzy_rho_fix[i]));
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsquared) {
        ...
        Pxy_fix[i] += exp_ij*SPH_weight_fact_ij*gamma_factor*f_prefactor*((dx_rho_fix[j]*dy_rho_fix[j])/rho_SPH[j] - 0.5*(dxy_rho_fix[j]+dyx_rho_fix[i]));
        Pxz_fix[i] += exp_ij*SPH_weight_fact_ij*gamma_factor*f_prefactor*((dx_rho_fix[j]*dz_rho_fix[j])/rho_SPH[j] - 0.5*(dxz_rho_fix[j]+dzx_rho_fix[i]));
        Pyy_fix[i] += exp_ij*SPH_weight_fact_ij*gamma_factor*f_prefactor*((dy_rho_fix[j]*dy_rho_fix[j])/rho_SPH[j] - dyy_rho_fix[j]);
        Pyz_fix[i] += exp_ij*SPH_weight_fact_ij*gamma_factor*f_prefactor*((dy_rho_fix[j]*dz_rho_fix[j])/rho_SPH[j] - 0.5*(dyz_rho_fix[j]+dzy_rho_fix[i]));
        ...
        if (newton_pair || j < nlocal) {
          ...
          Pxy_fix[j] += exp_ji*SPH_weight_fact_ji*gamma_factor*f_prefactor*((dx_rho_fix[i]*dy_rho_fix[i])/rho_SPH[i] - 0.5*(dxy_rho_fix[i]+dyx_rho_fix[i]));
          Pxz_fix[j] += exp_ji*SPH_weight_fact_ji*gamma_factor*f_prefactor*((dx_rho_fix[i]*dz_rho_fix[i])/rho_SPH[i] - 0.5*(dxz_rho_fix[i]+dzx_rho_fix[i]));
          Pyy_fix[j] += exp_ji*SPH_weight_fact_ji*gamma_factor*f_prefactor*((dy_rho_fix[i]*dy_rho_fix[i])/rho_SPH[i] - dyy_rho_fix[i]);
          Pyz_fix[j] += exp_ji*SPH_weight_fact_ji*gamma_factor*f_prefactor*((dy_rho_fix[i]*dz_rho_fix[i])/rho_SPH[i] - 0.5*(dyz_rho_fix[i]+dzy_rho_fix[i]));
...
int FixDynamicWidthsCoMBPDoubSymRobFtwo::pack_forward_comm(int n, int *list, double *buf,
...
void FixDynamicWidthsCoMBPDoubSymRobFtwo::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
...
int FixDynamicWidthsCoMBPDoubSymRobFtwo::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void FixDynamicWidthsCoMBPDoubSymRobFtwo::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
void FixDynamicWidthsCoMBPDoubSymRobFtwo::allocate()
{
  // // fprintf(screen,"\nFixDynamicWidthsCoMBPDoubSymRobFtwo::allocate()\n");
...
void FixDynamicWidthsCoMBPDoubSymRobFtwo::deallocate()
{
  // // fprintf(screen,"\nFixDynamicWidthsCoMBPDoubSymRobFtwo::deallocate()\n");
  delete [] mu;
  delete [] mu_all;
  delete [] tau;
  delete [] tau_all;
...
double FixDynamicWidthsCoMBPDoubSymRobFtwo::compute_scalar()
{
