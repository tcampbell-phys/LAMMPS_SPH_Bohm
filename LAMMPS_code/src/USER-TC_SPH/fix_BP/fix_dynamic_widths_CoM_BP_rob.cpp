...
#include "fix_dynamic_widths_CoM_BP_rob.h"
#include <cstring>
#include <mpi.h>
#include "group.h"
#include "modify.h"
...
FixDynamicWidthsCoMBPRob::FixDynamicWidthsCoMBPRob(LAMMPS *lmp, int narg, char **arg):
	Fix(lmp, narg, arg), mu(NULL), mu_all(NULL), tau(NULL), tau_all(NULL)
{
  ...
  // // fprintf(screen,"\nIn FixDynamicWidthsCoMBPRob with MPI_Allreduce and no commflag = 4 or 5");

  eflag = 0;
  eBohm = 0.0;
}

FixDynamicWidthsCoMBPRob::~FixDynamicWidthsCoMBPRob()
{
...
int FixDynamicWidthsCoMBPRob::setmask()
{
  int mask = 0;
...
void FixDynamicWidthsCoMBPRob::init()
{ 
...
void FixDynamicWidthsCoMBPRob::setup_pre_force(int)
{
  FixedPointIterator();
  CoM_Calculator();
}

void FixDynamicWidthsCoMBPRob::min_pre_force(int)
{
  FixedPointIterator();
  CoM_Calculator();
}
...
void FixDynamicWidthsCoMBPRob::setup_post_neighbor()
{
...
void FixDynamicWidthsCoMBPRob::post_neighbor()
{
  return;
}

void FixDynamicWidthsCoMBPRob::pre_force(int)
{
  FixedPointIterator();
  CoM_Calculator();
}

void FixDynamicWidthsCoMBPRob::end_of_step()
{
  // fprintf(screen,"\nFixDynamicWidthsCoMBPRob end_of_step");
  CalcBohmPot();
}

void FixDynamicWidthsCoMBPRob::FixedPointIterator()
{
...
double FixDynamicWidthsCoMBPRob::Gauss_Width_Deriv(double pre_fact, double wid, double sep_sq)
{
  return pre_fact*exp(-sep_sq/(wid*wid*2.0))*((sep_sq)/(wid*wid*wid) - 3.0/wid);
}

void FixDynamicWidthsCoMBPRob::CoM_Calculator()
{
  int i,ii,inum;
  int *ilist;
	double **x = atom->x;
  ...
  if (unallocated){
    deallocate();
    // // // fprintf(screen,"\nFixDynamicWidthsCoMBPRob allocating mu tau memory...\n");
    allocate();
    allocated = 1;
    unallocated = 0;
  }
...
void FixDynamicWidthsCoMBPRob::CalcBohmPot()
{
  // fprintf(screen,"\nCheckpoint 0");
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsquared) {
        ...
        dxx_rho_fix[i] += hm2_i*(delx_2*hm2_i - 1.)*m_gauss_ij;
        dxy_rho_fix[i] += hm4_i*(delx*dely)*m_gauss_ij;
        dxz_rho_fix[i] += hm4_i*(delx*delz)*m_gauss_ij;
        dyy_rho_fix[i] += hm2_i*(dely_2*hm2_i - 1.)*m_gauss_ij;
        dyz_rho_fix[i] += hm4_i*(dely*delz)*m_gauss_ij;
        dzz_rho_fix[i] += hm2_i*(delz_2*hm2_i - 1.)*m_gauss_ij;
        ...
        if (newton_pair || j < nlocal) {
          ...
          dxx_rho_fix[j] += hm2_j*(delx_2*hm2_j - 1.)*m_gauss_ji;
          dxy_rho_fix[j] += hm4_j*(delx*dely)*m_gauss_ji;
          dxz_rho_fix[j] += hm4_j*(delx*delz)*m_gauss_ji;
          dyy_rho_fix[j] += hm2_j*(dely_2*hm2_j - 1.)*m_gauss_ji;
          dyz_rho_fix[j] += hm4_j*(dely*delz)*m_gauss_ji;
          dzz_rho_fix[j] += hm2_j*(delz_2*hm2_j - 1.)*m_gauss_ji;
        }
      }
    }
  }
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    // if (tag[i] == tag_targ) {
    //   fprintf(screen,"\nfix_dynamic_widths_CoM_BP_rob tag_targ = %d",tag_targ);
...
int FixDynamicWidthsCoMBPRob::pack_forward_comm(int n, int *list, double *buf,
...
void FixDynamicWidthsCoMBPRob::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
...
int FixDynamicWidthsCoMBPRob::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void FixDynamicWidthsCoMBPRob::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
void FixDynamicWidthsCoMBPRob::allocate()
{
  // // fprintf(screen,"\nFixDynamicWidthsCoMBPRob::allocate()\n");
...
void FixDynamicWidthsCoMBPRob::deallocate()
{
  // // fprintf(screen,"\nFixDynamicWidthsCoMBPRob::deallocate()\n");
  delete [] mu;
  delete [] mu_all;
  delete [] tau;
  delete [] tau_all;
...
double FixDynamicWidthsCoMBPRob::compute_scalar()
{
