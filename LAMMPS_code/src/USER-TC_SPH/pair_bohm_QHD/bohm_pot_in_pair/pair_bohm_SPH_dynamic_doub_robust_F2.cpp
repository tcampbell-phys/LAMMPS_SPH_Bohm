...
Thomas Campbell
------------------------------------------------------------------------- */
#include "pair_bohm_SPH_dynamic_doub_robust_F2.h"
#include <mpi.h>
#include <cmath>
#include <cstring>
#include <math.h>
...
PairBohmSPHDynamicDoubRobustFtwo::PairBohmSPHDynamicDoubRobustFtwo(LAMMPS *lmp) : Pair(lmp) {
  nmax = 0;
...
PairBohmSPHDynamicDoubRobustFtwo::~PairBohmSPHDynamicDoubRobustFtwo()
{
  if (allocated) {
...
void PairBohmSPHDynamicDoubRobustFtwo::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsquared) {
        ...
        dxx_rho[i] += dx_rho[j]*((-delx)*hm2_i)*m_gauss_ij_rho;
        dxy_rho[i] += dy_rho[j]*((-delx)*hm2_i)*m_gauss_ij_rho;
        dxz_rho[i] += dz_rho[j]*((-delx)*hm2_i)*m_gauss_ij_rho;
        dyy_rho[i] += dy_rho[j]*((-dely)*hm2_i)*m_gauss_ij_rho;
        dyx_rho[i] += dx_rho[j]*((-dely)*hm2_i)*m_gauss_ij_rho;
        dyz_rho[i] += dz_rho[j]*((-dely)*hm2_i)*m_gauss_ij_rho;
        dzx_rho[i] += dx_rho[j]*((-delz)*hm2_i)*m_gauss_ij_rho;
        dzy_rho[i] += dy_rho[j]*((-delz)*hm2_i)*m_gauss_ij_rho;
        dzz_rho[i] += dz_rho[j]*((-delz)*hm2_i)*m_gauss_ij_rho;
        ...
        if (newton_pair || j < nlocal) {
          ...
          dxx_rho[j] -= dx_rho[i]*((-delx)*hm2_j)*m_gauss_ji_rho;
          dxy_rho[j] -= dy_rho[i]*((-delx)*hm2_j)*m_gauss_ji_rho;
          dxz_rho[j] -= dz_rho[i]*((-delx)*hm2_j)*m_gauss_ji_rho;
          dyy_rho[j] -= dy_rho[i]*((-dely)*hm2_j)*m_gauss_ji_rho;
          dyx_rho[j] -= dx_rho[i]*((-dely)*hm2_j)*m_gauss_ji_rho;
          dyz_rho[j] -= dz_rho[i]*((-dely)*hm2_j)*m_gauss_ji_rho;
          dzx_rho[j] -= dx_rho[i]*((-delz)*hm2_j)*m_gauss_ji_rho;
          dzy_rho[j] -= dy_rho[i]*((-delz)*hm2_j)*m_gauss_ji_rho;
          dzz_rho[j] -= dz_rho[i]*((-delz)*hm2_j)*m_gauss_ji_rho;
        }
      }
    }
  }
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    // if (tag[i] == tag_targ) {
    //   fprintf(screen,"\npair_bohm_SPH_dynamic_doub_robust_F2_fix tag_targ = %d",tag_targ);
    //   fprintf(screen,"\nPxx = %16.16f",Pxx[i]);
    //   fprintf(screen,"\nPxy = %16.16f",Pxy[i]);
    //   fprintf(screen,"\nPxz = %16.16f",Pxz[i]);
    //   fprintf(screen,"\nPyy = %16.16f",Pyy[i]);
...
void PairBohmSPHDynamicDoubRobustFtwo::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairBohmSPHDynamicDoubRobustFtwo::settings(int narg, char **arg)
{
...
void PairBohmSPHDynamicDoubRobustFtwo::coeff(int narg, char **arg)
{
  if (narg < 2 || narg > 3)
...
void PairBohmSPHDynamicDoubRobustFtwo::init_style()
{
...
double PairBohmSPHDynamicDoubRobustFtwo::init_one(int i, int j)
{
  if (setflag[i][j] == 0)
...
void PairBohmSPHDynamicDoubRobustFtwo::write_restart(FILE *fp)
{
...
void PairBohmSPHDynamicDoubRobustFtwo::read_restart(FILE *fp)
{
...
void PairBohmSPHDynamicDoubRobustFtwo::write_restart_settings(FILE *fp)
{
...
void PairBohmSPHDynamicDoubRobustFtwo::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
int PairBohmSPHDynamicDoubRobustFtwo::pack_forward_comm(int n, int *list, double *buf,
...
void PairBohmSPHDynamicDoubRobustFtwo::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
...
int PairBohmSPHDynamicDoubRobustFtwo::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairBohmSPHDynamicDoubRobustFtwo::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
void *PairBohmSPHDynamicDoubRobustFtwo::extract(const char *str, int &dim)
{
  dim = 2;
  return NULL;
}
