...
Thomas Campbell
------------------------------------------------------------------------- */
#include "pair_bohm_SPH_dynamic_doubtwo_robust_F2.h"
#include <mpi.h>
#include <cmath>
#include <cstring>
#include <math.h>
...
PairBohmSPHDynamicDoubtwoRobustFtwo::PairBohmSPHDynamicDoubtwoRobustFtwo(LAMMPS *lmp) : Pair(lmp) {
  nmax = 0;
...
PairBohmSPHDynamicDoubtwoRobustFtwo::~PairBohmSPHDynamicDoubtwoRobustFtwo()
{
  if (allocated) {
    ...
    memory->destroy(dyx_rho);
    memory->destroy(dzx_rho);
    memory->destroy(dzy_rho);
...
void PairBohmSPHDynamicDoubtwoRobustFtwo::compute(int eflag, int vflag)
{
  ...
  if (atom->nmax > nmax) {
    ...
    memory->create(dyx_rho,nmax,"pair:dyx_rho");
    memory->create(dzx_rho,nmax,"pair:dzx_rho");
    memory->create(dzy_rho,nmax,"pair:dzy_rho");
    ...
    memory->create(Pyx,nmax,"pair:Pyx");
    memory->create(Pzx,nmax,"pair:Pzx");
    memory->create(Pzy,nmax,"pair:Pzy");
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsquared) {
        ...
        dx_rho[i] += ((-delx)*hm2_i)*m_gauss_ij*dens_factor_ij;
        dy_rho[i] += ((-dely)*hm2_i)*m_gauss_ij*dens_factor_ij;
        dz_rho[i] += ((-delz)*hm2_i)*m_gauss_ij*dens_factor_ij;
        ...
        if (newton_pair || j < nlocal) {
          ...
          dx_rho[j] += ((delx)*hm2_j)*m_gauss_ji*dens_factor_ji;
          dy_rho[j] += ((dely)*hm2_j)*m_gauss_ji*dens_factor_ji;
          dz_rho[j] += ((delz)*hm2_j)*m_gauss_ji*dens_factor_ji;
        }
      }
    }
  }
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsquared) {
        ...
        dxx_rho[i] += (dx_rho[j]-dx_rho[i])*((-delx)*hm2_i)*m_gauss_ij_rho;
        dxy_rho[i] += (dy_rho[j]-dy_rho[i])*((-delx)*hm2_i)*m_gauss_ij_rho;
        dxz_rho[i] += (dz_rho[j]-dz_rho[i])*((-delx)*hm2_i)*m_gauss_ij_rho;
        dyy_rho[i] += (dy_rho[j]-dy_rho[i])*((-dely)*hm2_i)*m_gauss_ij_rho;
        dyx_rho[i] += (dx_rho[j]-dx_rho[i])*((-dely)*hm2_i)*m_gauss_ij_rho;
        dyz_rho[i] += (dz_rho[j]-dz_rho[i])*((-dely)*hm2_i)*m_gauss_ij_rho;
        dzx_rho[i] += (dx_rho[j]-dx_rho[i])*((-delz)*hm2_i)*m_gauss_ij_rho;
        dzy_rho[i] += (dy_rho[j]-dy_rho[i])*((-delz)*hm2_i)*m_gauss_ij_rho;
        dzz_rho[i] += (dz_rho[j]-dz_rho[i])*((-delz)*hm2_i)*m_gauss_ij_rho;
        ...
        if (newton_pair || j < nlocal) {
          ...
          dxx_rho[j] -= (dx_rho[i]-dx_rho[j])*((-delx)*hm2_j)*m_gauss_ji_rho;
          dxy_rho[j] -= (dy_rho[i]-dy_rho[j])*((-delx)*hm2_j)*m_gauss_ji_rho;
          dxz_rho[j] -= (dz_rho[i]-dz_rho[j])*((-delx)*hm2_j)*m_gauss_ji_rho;
          dyy_rho[j] -= (dy_rho[i]-dy_rho[j])*((-dely)*hm2_j)*m_gauss_ji_rho;
          dyx_rho[j] -= (dx_rho[i]-dx_rho[j])*((-dely)*hm2_j)*m_gauss_ji_rho;
          dyz_rho[j] -= (dz_rho[i]-dz_rho[j])*((-dely)*hm2_j)*m_gauss_ji_rho;
          dzx_rho[j] -= (dx_rho[i]-dx_rho[j])*((-delz)*hm2_j)*m_gauss_ji_rho;
          dzy_rho[j] -= (dy_rho[i]-dy_rho[j])*((-delz)*hm2_j)*m_gauss_ji_rho;
          dzz_rho[j] -= (dz_rho[i]-dz_rho[j])*((-delz)*hm2_j)*m_gauss_ji_rho;
        }
      }
    }
  }
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    Pyx[i] += SPH_weight_fact_self*gamma_factor*f_prefactor*((dy_rho[i]*dx_rho[i])/rho_SPH[i] - dyx_rho[i]);
    Pzx[i] += SPH_weight_fact_self*gamma_factor*f_prefactor*((dz_rho[i]*dx_rho[i])/rho_SPH[i] - dzx_rho[i]);
    Pzy[i] += SPH_weight_fact_self*gamma_factor*f_prefactor*((dz_rho[i]*dy_rho[i])/rho_SPH[i] - dzy_rho[i]);
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsquared) {
        ...
        Pyx[i] += exp_ij*SPH_weight_fact_ij*gamma_factor*f_prefactor*((dy_rho[j]*dx_rho[j])/rho_SPH[j] - dyx_rho[j]);
        Pzx[i] += exp_ij*SPH_weight_fact_ij*gamma_factor*f_prefactor*((dz_rho[j]*dx_rho[j])/rho_SPH[j] - dzx_rho[j]);
        Pzy[i] += exp_ij*SPH_weight_fact_ij*gamma_factor*f_prefactor*((dz_rho[j]*dy_rho[j])/rho_SPH[j] - dzy_rho[j]);
        ...
        if (newton_pair || j < nlocal) {
          ...
          Pyx[j] += exp_ji*SPH_weight_fact_ji*gamma_factor*f_prefactor*((dy_rho[i]*dx_rho[i])/rho_SPH[i] - dyx_rho[i]);
          Pzx[j] += exp_ji*SPH_weight_fact_ji*gamma_factor*f_prefactor*((dz_rho[i]*dx_rho[i])/rho_SPH[i] - dzx_rho[i]);
          Pzy[j] += exp_ji*SPH_weight_fact_ji*gamma_factor*f_prefactor*((dz_rho[i]*dy_rho[i])/rho_SPH[i] - dzy_rho[i]);
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    // if (tag[i] == tag_targ) {
    //   fprintf(screen,"\npair_bohm_SPH_dynamic_doubtwo_robust_F2_fix tag_targ = %d",tag_targ);
    //   fprintf(screen,"\nPxx = %16.16f",Pxx[i]);
    //   fprintf(screen,"\nPxy = %16.16f",Pxy[i]);
    //   fprintf(screen,"\nPxz = %16.16f",Pxz[i]);
    //   fprintf(screen,"\nPyy = %16.16f",Pyy[i]);
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsquared) {
        ...
        u_SPH[i] += u_prefact_i*ijmass*((Pxx[i]*delvx + Pxy[i]*delvy + Pxz[i]*delvz)*dx_Wij + (Pyx[i]*delvx + Pyy[i]*delvy + Pyz[i]*delvz)*dy_Wij + (Pzx[i]*delvx + Pzy[i]*delvy + Pzz[i]*delvz)*dz_Wij);
        ...
        fy = -ijmass*((Pyx[i]*dx_Wij + Pyy[i]*dy_Wij + Pyz[i]*dz_Wij)/(rho_i2*omega_i) + (Pyx[j]*dx_Wji + Pyy[j]*dy_Wji + Pyz[j]*dz_Wji)/(rho_j2*omega_j));
        fz = -ijmass*((Pzx[i]*dx_Wij + Pzy[i]*dy_Wij + Pzz[i]*dz_Wij)/(rho_i2*omega_i) + (Pzx[j]*dx_Wji + Pzy[j]*dy_Wji + Pzz[j]*dz_Wji)/(rho_j2*omega_j));
        ...
        if (newton_pair || j < nlocal) {
          ...
          u_SPH[j] -= u_prefact_j*ijmass*((-Pxx[j]*delvx - Pxy[j]*delvy - Pxz[j]*delvz)*dx_Wji + (-Pyx[j]*delvx - Pyy[j]*delvy - Pyz[j]*delvz)*dy_Wji + (-Pzx[j]*delvx - Pzx[j]*delvy - Pzz[j]*delvz)*dz_Wji);
        }
...
void PairBohmSPHDynamicDoubtwoRobustFtwo::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairBohmSPHDynamicDoubtwoRobustFtwo::settings(int narg, char **arg)
{
...
void PairBohmSPHDynamicDoubtwoRobustFtwo::coeff(int narg, char **arg)
{
  if (narg < 2 || narg > 3)
...
void PairBohmSPHDynamicDoubtwoRobustFtwo::init_style()
{
...
double PairBohmSPHDynamicDoubtwoRobustFtwo::init_one(int i, int j)
{
  if (setflag[i][j] == 0)
...
void PairBohmSPHDynamicDoubtwoRobustFtwo::write_restart(FILE *fp)
{
...
void PairBohmSPHDynamicDoubtwoRobustFtwo::read_restart(FILE *fp)
{
...
void PairBohmSPHDynamicDoubtwoRobustFtwo::write_restart_settings(FILE *fp)
{
...
void PairBohmSPHDynamicDoubtwoRobustFtwo::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
int PairBohmSPHDynamicDoubtwoRobustFtwo::pack_forward_comm(int n, int *list, double *buf,
...
void PairBohmSPHDynamicDoubtwoRobustFtwo::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
...
int PairBohmSPHDynamicDoubtwoRobustFtwo::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairBohmSPHDynamicDoubtwoRobustFtwo::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
void *PairBohmSPHDynamicDoubtwoRobustFtwo::extract(const char *str, int &dim)
{
  dim = 2;
  return NULL;
}
