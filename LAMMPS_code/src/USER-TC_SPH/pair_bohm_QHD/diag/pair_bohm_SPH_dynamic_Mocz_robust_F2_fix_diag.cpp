...
Thomas Campbell
------------------------------------------------------------------------- */
#include "pair_bohm_SPH_dynamic_Mocz_robust_F2_fix_diag.h"
#include <mpi.h>
#include <cmath>
#include <cstring>
#include <math.h>
...
PairBohmSPHDynamicMoczRobustFtwoFixDiag::PairBohmSPHDynamicMoczRobustFtwoFixDiag(LAMMPS *lmp) : Pair(lmp) {
  nmax = 0;
...
PairBohmSPHDynamicMoczRobustFtwoFixDiag::~PairBohmSPHDynamicMoczRobustFtwoFixDiag()
{
  if (allocated) {
...
void PairBohmSPHDynamicMoczRobustFtwoFixDiag::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    // if (tag[i] == tag_targ) {
    //   fprintf(screen,"\npair_bohm_SPH_dynamic_Mocz_robust_F2_fix tag_targ = %d",tag_targ);
    //   fprintf(screen,"\nPxx = %16.16f",Pxx[i]);
    //   fprintf(screen,"\nPxy = %16.16f",Pxy[i]);
    //   fprintf(screen,"\nPxz = %16.16f",Pxz[i]);
    //   fprintf(screen,"\nPyy = %16.16f",Pyy[i]);
...
void PairBohmSPHDynamicMoczRobustFtwoFixDiag::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairBohmSPHDynamicMoczRobustFtwoFixDiag::settings(int narg, char **arg)
{
...
void PairBohmSPHDynamicMoczRobustFtwoFixDiag::coeff(int narg, char **arg)
{
  if (narg < 2 || narg > 3)
...
void PairBohmSPHDynamicMoczRobustFtwoFixDiag::init_style()
{
...
double PairBohmSPHDynamicMoczRobustFtwoFixDiag::init_one(int i, int j)
{
  if (setflag[i][j] == 0)
...
void PairBohmSPHDynamicMoczRobustFtwoFixDiag::write_restart(FILE *fp)
{
...
void PairBohmSPHDynamicMoczRobustFtwoFixDiag::read_restart(FILE *fp)
{
...
void PairBohmSPHDynamicMoczRobustFtwoFixDiag::write_restart_settings(FILE *fp)
{
...
void PairBohmSPHDynamicMoczRobustFtwoFixDiag::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
int PairBohmSPHDynamicMoczRobustFtwoFixDiag::pack_forward_comm(int n, int *list, double *buf,
...
void PairBohmSPHDynamicMoczRobustFtwoFixDiag::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
...
int PairBohmSPHDynamicMoczRobustFtwoFixDiag::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairBohmSPHDynamicMoczRobustFtwoFixDiag::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
void *PairBohmSPHDynamicMoczRobustFtwoFixDiag::extract(const char *str, int &dim)
{
  dim = 2;
  return NULL;
}
