...
Thomas Campbell
------------------------------------------------------------------------- */
#include "pair_bohm_SPH_dynamic_Mocz_robust_vest.h"
#include <mpi.h>
#include <cmath>
#include <cstring>
#include <math.h>
...
PairBohmSPHDynamicMoczRobustVest::PairBohmSPHDynamicMoczRobustVest(LAMMPS *lmp) : Pair(lmp) {
  nmax = 0;
...
PairBohmSPHDynamicMoczRobustVest::~PairBohmSPHDynamicMoczRobustVest()
{
  if (allocated) {
...
void PairBohmSPHDynamicMoczRobustVest::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsquared) {
        ...
        delvx = vxtmp - vest[j][0];
        delvy = vytmp - vest[j][1];
        delvz = vztmp - vest[j][2];
        ...
        u_SPH[i] += u_prefact_i*ijmass*((Pxx[i]*delvx + Pxy[i]*delvy + Pxz[i]*delvz)*dx_Wij + (Pxy[i]*delvx + Pyy[i]*delvy + Pyz[i]*delvz)*dy_Wij + (Pxz[i]*delvx + Pyz[i]*delvy + Pzz[i]*delvz)*dz_Wij);
        ...
        if (newton_pair || j < nlocal) {
          ...
          u_SPH[j] -= u_prefact_j*ijmass*((-Pxx[j]*delvx - Pxy[j]*delvy - Pxz[j]*delvz)*dx_Wji + (-Pxy[j]*delvx - Pyy[j]*delvy - Pyz[j]*delvz)*dy_Wji + (-Pxz[j]*delvx - Pyz[j]*delvy - Pzz[j]*delvz)*dz_Wji);
        }
...
void PairBohmSPHDynamicMoczRobustVest::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairBohmSPHDynamicMoczRobustVest::settings(int narg, char **arg)
{
...
void PairBohmSPHDynamicMoczRobustVest::coeff(int narg, char **arg)
{
  if (narg < 2 || narg > 3)
...
void PairBohmSPHDynamicMoczRobustVest::init_style()
{
...
double PairBohmSPHDynamicMoczRobustVest::init_one(int i, int j)
{
  if (setflag[i][j] == 0)
...
void PairBohmSPHDynamicMoczRobustVest::write_restart(FILE *fp)
{
...
void PairBohmSPHDynamicMoczRobustVest::read_restart(FILE *fp)
{
...
void PairBohmSPHDynamicMoczRobustVest::write_restart_settings(FILE *fp)
{
...
void PairBohmSPHDynamicMoczRobustVest::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
int PairBohmSPHDynamicMoczRobustVest::pack_forward_comm(int n, int *list, double *buf,
...
void PairBohmSPHDynamicMoczRobustVest::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
...
int PairBohmSPHDynamicMoczRobustVest::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairBohmSPHDynamicMoczRobustVest::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
void *PairBohmSPHDynamicMoczRobustVest::extract(const char *str, int &dim)
{
  dim = 2;
  return NULL;
}
