...
Thomas Campbell
------------------------------------------------------------------------- */
#include "pair_bohm_SPH_dynamic_Mocz_robust_pot.h"
#include <mpi.h>
#include <cmath>
#include <cstring>
#include <math.h>
...
PairBohmSPHDynamicMoczRobustPot::PairBohmSPHDynamicMoczRobustPot(LAMMPS *lmp) : Pair(lmp) {
  nmax = 0;
...
PairBohmSPHDynamicMoczRobustPot::~PairBohmSPHDynamicMoczRobustPot()
{
  if (allocated) {
...
void PairBohmSPHDynamicMoczRobustPot::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    Pxx[i] += SPH_weight_fact_self*gamma_factor*f_prefactor*((dx_rho_SPH[i]*dx_rho_SPH[i])/rho_SPH[i] - dxx_rho[i]);
    Pxy[i] += SPH_weight_fact_self*gamma_factor*f_prefactor*((dx_rho_SPH[i]*dy_rho_SPH[i])/rho_SPH[i] - dxy_rho[i]);
    Pxz[i] += SPH_weight_fact_self*gamma_factor*f_prefactor*((dx_rho_SPH[i]*dz_rho_SPH[i])/rho_SPH[i] - dxz_rho[i]);
    Pyy[i] += SPH_weight_fact_self*gamma_factor*f_prefactor*((dy_rho_SPH[i]*dy_rho_SPH[i])/rho_SPH[i] - dyy_rho[i]);
    Pyz[i] += SPH_weight_fact_self*gamma_factor*f_prefactor*((dy_rho_SPH[i]*dz_rho_SPH[i])/rho_SPH[i] - dyz_rho[i]);
    Pzz[i] += SPH_weight_fact_self*gamma_factor*f_prefactor*((dz_rho_SPH[i]*dz_rho_SPH[i])/rho_SPH[i] - dzz_rho[i]);

    u_SPH[i] += -0.5*f_prefactor*SPH_weight_fact_self*(2*(dxx_rho[i] + dyy_rho[i] + dzz_rho[i])/rho_SPH[i] - (dx_rho_SPH[i]*dx_rho_SPH[i] + dy_rho_SPH[i]*dy_rho_SPH[i] + dz_rho_SPH[i]*dz_rho_SPH[i])/(rho_SPH[i]*rho_SPH[i]));
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsquared) {
        ...
        Pxx[i] += exp_ij*SPH_weight_fact_ij*gamma_factor*f_prefactor*((dx_rho_SPH[j]*dx_rho_SPH[j])/rho_SPH[j] - dxx_rho[j]);
        Pxy[i] += exp_ij*SPH_weight_fact_ij*gamma_factor*f_prefactor*((dx_rho_SPH[j]*dy_rho_SPH[j])/rho_SPH[j] - dxy_rho[j]);
        Pxz[i] += exp_ij*SPH_weight_fact_ij*gamma_factor*f_prefactor*((dx_rho_SPH[j]*dz_rho_SPH[j])/rho_SPH[j] - dxz_rho[j]);
        Pyy[i] += exp_ij*SPH_weight_fact_ij*gamma_factor*f_prefactor*((dy_rho_SPH[j]*dy_rho_SPH[j])/rho_SPH[j] - dyy_rho[j]);
        Pyz[i] += exp_ij*SPH_weight_fact_ij*gamma_factor*f_prefactor*((dy_rho_SPH[j]*dz_rho_SPH[j])/rho_SPH[j] - dyz_rho[j]);
        Pzz[i] += exp_ij*SPH_weight_fact_ij*gamma_factor*f_prefactor*((dz_rho_SPH[j]*dz_rho_SPH[j])/rho_SPH[j] - dzz_rho[j]);

        u_SPH[i] += -0.5*f_prefactor*exp_ij*SPH_weight_fact_ij*(2*(dxx_rho[j] + dyy_rho[j] + dzz_rho[j])/rho_SPH[j] - (dx_rho_SPH[j]*dx_rho_SPH[j] + dy_rho_SPH[j]*dy_rho_SPH[j] + dz_rho_SPH[j]*dz_rho_SPH[j])/(rho_SPH[j]*rho_SPH[j]));
        ...
        if (newton_pair || j < nlocal) {
          ...
          Pxx[j] += exp_ji*SPH_weight_fact_ji*gamma_factor*f_prefactor*((dx_rho_SPH[i]*dx_rho_SPH[i])/rho_SPH[i] - dxx_rho[i]);
          Pxy[j] += exp_ji*SPH_weight_fact_ji*gamma_factor*f_prefactor*((dx_rho_SPH[i]*dy_rho_SPH[i])/rho_SPH[i] - dxy_rho[i]);
          Pxz[j] += exp_ji*SPH_weight_fact_ji*gamma_factor*f_prefactor*((dx_rho_SPH[i]*dz_rho_SPH[i])/rho_SPH[i] - dxz_rho[i]);
          Pyy[j] += exp_ji*SPH_weight_fact_ji*gamma_factor*f_prefactor*((dy_rho_SPH[i]*dy_rho_SPH[i])/rho_SPH[i] - dyy_rho[i]);
          Pyz[j] += exp_ji*SPH_weight_fact_ji*gamma_factor*f_prefactor*((dy_rho_SPH[i]*dz_rho_SPH[i])/rho_SPH[i] - dyz_rho[i]);
          Pzz[j] += exp_ji*SPH_weight_fact_ji*gamma_factor*f_prefactor*((dz_rho_SPH[i]*dz_rho_SPH[i])/rho_SPH[i] - dzz_rho[i]);

          u_SPH[j] += -0.5*f_prefactor*exp_ij*SPH_weight_fact_ij*(2*(dxx_rho[i] + dyy_rho[i] + dzz_rho[i])/rho_SPH[i] - (dx_rho_SPH[i]*dx_rho_SPH[i] + dy_rho_SPH[i]*dy_rho_SPH[i] + dz_rho_SPH[i]*dz_rho_SPH[i])/(rho_SPH[i]*rho_SPH[i]));
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    // u_SPH[i] = -0.5*f_prefactor*(2*(dxx_rho[i] + dyy_rho[i] + dzz_rho[i])/rho_SPH[i] - (dx_rho_SPH[i]*dx_rho_SPH[i] + dy_rho_SPH[i]*dy_rho_SPH[i] + dz_rho_SPH[i]*dz_rho_SPH[i])/(rho_i2));
  ...
  for (ii = 0; ii < inum; ii++) {
    i = ilist[ii];
    if (eflag_global) eng_vdwl +=  u_SPH[i]/elements_per_electron;
    if (eflag_atom) eatom[i] += u_SPH[i]/elements_per_electron;
  }
...
void PairBohmSPHDynamicMoczRobustPot::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairBohmSPHDynamicMoczRobustPot::settings(int narg, char **arg)
{
  ...
  elements_per_electron = force->numeric(FLERR,arg[2]);
...
void PairBohmSPHDynamicMoczRobustPot::coeff(int narg, char **arg)
{
  if (narg < 2 || narg > 3)
...
void PairBohmSPHDynamicMoczRobustPot::init_style()
{
...
double PairBohmSPHDynamicMoczRobustPot::init_one(int i, int j)
{
  if (setflag[i][j] == 0)
...
void PairBohmSPHDynamicMoczRobustPot::write_restart(FILE *fp)
{
...
void PairBohmSPHDynamicMoczRobustPot::read_restart(FILE *fp)
{
...
void PairBohmSPHDynamicMoczRobustPot::write_restart_settings(FILE *fp)
{
...
void PairBohmSPHDynamicMoczRobustPot::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
int PairBohmSPHDynamicMoczRobustPot::pack_forward_comm(int n, int *list, double *buf,
...
void PairBohmSPHDynamicMoczRobustPot::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
...
int PairBohmSPHDynamicMoczRobustPot::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairBohmSPHDynamicMoczRobustPot::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
void *PairBohmSPHDynamicMoczRobustPot::extract(const char *str, int &dim)
{
  dim = 2;
  return NULL;
}
