...
Thomas Campbell
------------------------------------------------------------------------- */
#include "pair_bohm_SPH_dynamic_Mocz_robust_F2_traj.h"
#include <mpi.h>
#include <cmath>
#include <cstring>
#include <math.h>
...
PairBohmSPHDynamicMoczRobustFtwoTraj::PairBohmSPHDynamicMoczRobustFtwoTraj(LAMMPS *lmp) : Pair(lmp) {
  nmax = 0;
...
PairBohmSPHDynamicMoczRobustFtwoTraj::~PairBohmSPHDynamicMoczRobustFtwoTraj()
{
  if (allocated) {
...
void PairBohmSPHDynamicMoczRobustFtwoTraj::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsquared) {
        ...
        u_SPH[i] += occupation_fact*u_prefact_i*ijmass*((Pxx[i]*delvx + Pxy[i]*delvy + Pxz[i]*delvz)*dx_Wij + (Pxy[i]*delvx + Pyy[i]*delvy + Pyz[i]*delvz)*dy_Wij + (Pxz[i]*delvx + Pyz[i]*delvy + Pzz[i]*delvz)*dz_Wij);
        ...
        if (newton_pair || j < nlocal) {
          ...
          u_SPH[j] -= occupation_fact*u_prefact_j*ijmass*((-Pxx[j]*delvx - Pxy[j]*delvy - Pxz[j]*delvz)*dx_Wji + (-Pxy[j]*delvx - Pyy[j]*delvy - Pyz[j]*delvz)*dy_Wji + (-Pxz[j]*delvx - Pyz[j]*delvy - Pzz[j]*delvz)*dz_Wji);
        }
...
void PairBohmSPHDynamicMoczRobustFtwoTraj::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairBohmSPHDynamicMoczRobustFtwoTraj::settings(int narg, char **arg)
{
  ...
  occupation_fact = force->numeric(FLERR,arg[2]);
...
void PairBohmSPHDynamicMoczRobustFtwoTraj::coeff(int narg, char **arg)
{
  if (narg < 2 || narg > 3)
...
void PairBohmSPHDynamicMoczRobustFtwoTraj::init_style()
{
...
double PairBohmSPHDynamicMoczRobustFtwoTraj::init_one(int i, int j)
{
  if (setflag[i][j] == 0)
...
void PairBohmSPHDynamicMoczRobustFtwoTraj::write_restart(FILE *fp)
{
...
void PairBohmSPHDynamicMoczRobustFtwoTraj::read_restart(FILE *fp)
{
...
void PairBohmSPHDynamicMoczRobustFtwoTraj::write_restart_settings(FILE *fp)
{
...
void PairBohmSPHDynamicMoczRobustFtwoTraj::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
int PairBohmSPHDynamicMoczRobustFtwoTraj::pack_forward_comm(int n, int *list, double *buf,
...
void PairBohmSPHDynamicMoczRobustFtwoTraj::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
...
int PairBohmSPHDynamicMoczRobustFtwoTraj::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairBohmSPHDynamicMoczRobustFtwoTraj::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
void *PairBohmSPHDynamicMoczRobustFtwoTraj::extract(const char *str, int &dim)
{
  dim = 2;
  return NULL;
}
