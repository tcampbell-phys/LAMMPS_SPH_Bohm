...
Thomas Campbell
------------------------------------------------------------------------- */
#include "pair_bohm_SPH_dynamic_Mocz_robust_fix.h"
#include <mpi.h>
#include <cmath>
#include <cstring>
#include <math.h>
...
PairBohmSPHDynamicMoczRobustFix::PairBohmSPHDynamicMoczRobustFix(LAMMPS *lmp) : Pair(lmp) {
  nmax = 0;
...
PairBohmSPHDynamicMoczRobustFix::~PairBohmSPHDynamicMoczRobustFix()
{
  if (allocated) {
...
void PairBohmSPHDynamicMoczRobustFix::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    // if (tag[i] == tag_targ) {
    //   fprintf(screen,"\npair_bohm_SPH_dynamic_Mocz_robust_fix tag_targ = %d",tag_targ);
    //   fprintf(screen,"\nPxx = %16.16f",Pxx[i]);
    //   fprintf(screen,"\nPxy = %16.16f",Pxy[i]);
    //   fprintf(screen,"\nPxz = %16.16f",Pxz[i]);
    //   fprintf(screen,"\nPyy = %16.16f",Pyy[i]);
...
void PairBohmSPHDynamicMoczRobustFix::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairBohmSPHDynamicMoczRobustFix::settings(int narg, char **arg)
{
...
void PairBohmSPHDynamicMoczRobustFix::coeff(int narg, char **arg)
{
  if (narg < 2 || narg > 3)
...
void PairBohmSPHDynamicMoczRobustFix::init_style()
{
...
double PairBohmSPHDynamicMoczRobustFix::init_one(int i, int j)
{
  if (setflag[i][j] == 0)
...
void PairBohmSPHDynamicMoczRobustFix::write_restart(FILE *fp)
{
...
void PairBohmSPHDynamicMoczRobustFix::read_restart(FILE *fp)
{
...
void PairBohmSPHDynamicMoczRobustFix::write_restart_settings(FILE *fp)
{
...
void PairBohmSPHDynamicMoczRobustFix::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
int PairBohmSPHDynamicMoczRobustFix::pack_forward_comm(int n, int *list, double *buf,
...
void PairBohmSPHDynamicMoczRobustFix::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
...
int PairBohmSPHDynamicMoczRobustFix::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairBohmSPHDynamicMoczRobustFix::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
void *PairBohmSPHDynamicMoczRobustFix::extract(const char *str, int &dim)
{
  dim = 2;
  return NULL;
}
