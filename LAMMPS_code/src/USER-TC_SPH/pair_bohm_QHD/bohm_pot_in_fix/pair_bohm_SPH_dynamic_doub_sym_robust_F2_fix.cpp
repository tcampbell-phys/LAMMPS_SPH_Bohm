...
Thomas Campbell
------------------------------------------------------------------------- */
#include "pair_bohm_SPH_dynamic_doub_sym_robust_F2_fix.h"
#include <mpi.h>
#include <cmath>
#include <cstring>
#include <math.h>
...
PairBohmSPHDynamicDoubSymRobustFtwoFix::PairBohmSPHDynamicDoubSymRobustFtwoFix(LAMMPS *lmp) : Pair(lmp) {
  nmax = 0;
...
PairBohmSPHDynamicDoubSymRobustFtwoFix::~PairBohmSPHDynamicDoubSymRobustFtwoFix()
{
  if (allocated) {
...
void PairBohmSPHDynamicDoubSymRobustFtwoFix::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    Pxy[i] += SPH_weight_fact_self*gamma_factor*f_prefactor*((dx_rho[i]*dy_rho[i])/rho_SPH[i] - 0.5*(dxy_rho[i]+dyx_rho[i]));
    Pxz[i] += SPH_weight_fact_self*gamma_factor*f_prefactor*((dx_rho[i]*dz_rho[i])/rho_SPH[i] - 0.5*(dxz_rho[i]+dzx_rho[i]));
    Pyy[i] += SPH_weight_fact_self*gamma_factor*f_prefactor*((dy_rho[i]*dy_rho[i])/rho_SPH[i] - dyy_rho[i]);
    Pyz[i] += SPH_weight_fact_self*gamma_factor*f_prefactor*((dy_rho[i]*dz_rho[i])/rho_SPH[i] - 0.5*(dyz_rho[i]+dzy_rho[i]));
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsquared) {
        ...
        Pxy[i] += exp_ij*SPH_weight_fact_ij*gamma_factor*f_prefactor*((dx_rho[j]*dy_rho[j])/rho_SPH[j] - 0.5*(dxy_rho[j]+dyx_rho[j]));
        Pxz[i] += exp_ij*SPH_weight_fact_ij*gamma_factor*f_prefactor*((dx_rho[j]*dz_rho[j])/rho_SPH[j] - 0.5*(dxz_rho[j]+dzx_rho[j]));
        Pyy[i] += exp_ij*SPH_weight_fact_ij*gamma_factor*f_prefactor*((dy_rho[j]*dy_rho[j])/rho_SPH[j] - dyy_rho[j]);
        Pyz[i] += exp_ij*SPH_weight_fact_ij*gamma_factor*f_prefactor*((dy_rho[j]*dz_rho[j])/rho_SPH[j] - 0.5*(dyz_rho[j]+dzy_rho[j]));
        ...
        if (newton_pair || j < nlocal) {
          ...
          Pxy[j] += exp_ji*SPH_weight_fact_ji*gamma_factor*f_prefactor*((dx_rho[i]*dy_rho[i])/rho_SPH[i] - 0.5*(dxy_rho[i]+dyx_rho[i]));
          Pxz[j] += exp_ji*SPH_weight_fact_ji*gamma_factor*f_prefactor*((dx_rho[i]*dz_rho[i])/rho_SPH[i] - 0.5*(dxz_rho[i]+dzx_rho[i]));
          Pyy[j] += exp_ji*SPH_weight_fact_ji*gamma_factor*f_prefactor*((dy_rho[i]*dy_rho[i])/rho_SPH[i] - dyy_rho[i]);
          Pyz[j] += exp_ji*SPH_weight_fact_ji*gamma_factor*f_prefactor*((dy_rho[i]*dz_rho[i])/rho_SPH[i] - 0.5*(dyz_rho[i]+dzy_rho[i]));
...
void PairBohmSPHDynamicDoubSymRobustFtwoFix::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairBohmSPHDynamicDoubSymRobustFtwoFix::settings(int narg, char **arg)
{
...
void PairBohmSPHDynamicDoubSymRobustFtwoFix::coeff(int narg, char **arg)
{
  if (narg < 2 || narg > 3)
...
void PairBohmSPHDynamicDoubSymRobustFtwoFix::init_style()
{
...
double PairBohmSPHDynamicDoubSymRobustFtwoFix::init_one(int i, int j)
{
  if (setflag[i][j] == 0)
...
void PairBohmSPHDynamicDoubSymRobustFtwoFix::write_restart(FILE *fp)
{
...
void PairBohmSPHDynamicDoubSymRobustFtwoFix::read_restart(FILE *fp)
{
...
void PairBohmSPHDynamicDoubSymRobustFtwoFix::write_restart_settings(FILE *fp)
{
...
void PairBohmSPHDynamicDoubSymRobustFtwoFix::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
int PairBohmSPHDynamicDoubSymRobustFtwoFix::pack_forward_comm(int n, int *list, double *buf,
...
void PairBohmSPHDynamicDoubSymRobustFtwoFix::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
...
int PairBohmSPHDynamicDoubSymRobustFtwoFix::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairBohmSPHDynamicDoubSymRobustFtwoFix::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
void *PairBohmSPHDynamicDoubSymRobustFtwoFix::extract(const char *str, int &dim)
{
  dim = 2;
  return NULL;
}
