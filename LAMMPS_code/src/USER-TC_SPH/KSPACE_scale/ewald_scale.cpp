...
EwaldScale::EwaldScale(LAMMPS *lmp) : KSpace(lmp),
...
void EwaldScale::settings(int narg, char **arg)
{
  ...
  fprintf(screen,"\nEwaldScale occupation_fact = %f",occupation_fact);
  fprintf(screen,"\nEwaldScale ele_type = %d",ele_type);
...
void EwaldScale::init()
{
  if (comm->me == 0) {
    if (screen) fprintf(screen,"EwaldScale initialization ...\n");
    if (logfile) fprintf(logfile,"EwaldScale initialization ...\n");
  }
  ...
  if (domain->dimension == 2)
    error->all(FLERR,"Cannot use EwaldScale with 2d simulation");

  if (!atom->q_flag) error->all(FLERR,"Kspace style requires atom attribute q");

  if (slabflag == 0 && domain->nonperiodic > 0)
    error->all(FLERR,"Cannot use non-periodic boundaries with EwaldScale");
  if (slabflag) {
    if (domain->xperiodic != 1 || domain->yperiodic != 1 ||
        domain->boundary[2][0] != 1 || domain->boundary[2][1] != 1)
      error->all(FLERR,"Incorrect boundaries with slab EwaldScale");
    if (domain->triclinic)
      error->all(FLERR,"Cannot (yet) use EwaldScale with triclinic box "
                 "and slab correction");
  }
  ...
  // adjust z dimension for 2d slab EwaldScale
  // 3d EwaldScale just uses zprd since slab_volfactor = 1.0
  ...
  // setup EwaldScale coefficients so can print stats
...
/* ----------------------------------------------------------------------
   adjust EwaldScale coeffs, called initially and whenever volume has changed
------------------------------------------------------------------------- */

void EwaldScale::setup()
{
  ...
  // adjustment of z dimension for 2d slab EwaldScale
  // 3d EwaldScale just uses zprd since slab_volfactor = 1.0
  ...
  // pre-compute EwaldScale coefficients
...
double EwaldScale::rms(int km, double prd, bigint natoms, double q2)
{
...
/* ----------------------------------------------------------------------
   compute the EwaldScale long-range force, energy, virial
------------------------------------------------------------------------- */

void EwaldScale::compute(int eflag, int vflag)
{
  int i,j,k;
...
void EwaldScale::eik_dot_r_triclinic()
{
  int i,k,l,m,n,ic;
  double cstr1,sstr1;
  double sqk,clpm,slpm;
...
/* ----------------------------------------------------------------------
   pre-compute coefficients for each EwaldScale K-vector
------------------------------------------------------------------------- */

void EwaldScale::coeffs()
{
  int k,l,m;
  double sqk,vterm;
...
/* ----------------------------------------------------------------------
   pre-compute coefficients for each EwaldScale K-vector for a triclinic
   system
------------------------------------------------------------------------- */

void EwaldScale::coeffs_triclinic()
{
  int k,l,m;
  double sqk,vterm;
...
/* ----------------------------------------------------------------------
   ...
   periodically repeating slabs.  Yields good approximation to 2D EwaldScale if
   adequate empty space is left between repeating slabs (J. Chem. Phys.
   111, 3155).  Slabs defined here to be parallel to the xy plane. Also
   extended to non-neutral systems (J. Chem. Phys. 131, 094107).
------------------------------------------------------------------------- */

void EwaldScale::slabcorr()
{
...
double EwaldScale::memory_usage()
{
...
/* ----------------------------------------------------------------------
   compute the EwaldScale total long-range force and energy for groups A and B
 ------------------------------------------------------------------------- */

void EwaldScale::compute_group_group(int groupbit_A, int groupbit_B, int AA_flag)
{
...
void EwaldScale::slabcorr_groups(int groupbit_A, int groupbit_B, int AA_flag)
{
  // compute local contribution to global dipole moment
...
void EwaldScale::allocate_groups()
{
  // group A
...
void EwaldScale::deallocate_groups()
{
  // group A
