...
KSpaceStyle(ewald_scale,EwaldScale)

#else

#ifndef LMP_EWALD_SCALE_H
#define LMP_EWALD_SCALE_H

#include "kspace.h"

namespace LAMMPS_NS {

class EwaldScale : public KSpace {
 public:
  EwaldScale(class LAMMPS *);
  virtual ~EwaldScale();
  void init();
  void setup();
...
E: Cannot use EwaldScale with 2d simulation

The kspace style ewald cannot be used in 2d simulations.  You can use
2d EwaldScale in a 3d simulation; see the kspace_modify command.

E: Kspace style requires atom attribute q

The atom style defined does not have these attributes.

E: Cannot use non-periodic boundaries with EwaldScale

For kspace style ewald, all 3 dimensions must have periodic boundaries
unless you use the kspace_modify command to define a 2d slab with a
non-periodic z dimension.

E: Incorrect boundaries with slab EwaldScale

Must have periodic x,y dimensions and non-periodic z dimension to use
2d slab option with EwaldScale.

E: Cannot (yet) use EwaldScale with triclinic box and slab correction
