...
PairCoulLongSPH::PairCoulLongSPH(LAMMPS *lmp) : Pair(lmp)
{
  ...
  dx_rho_rev_coul = NULL;
  dy_rho_rev_coul = NULL;
  dz_rho_rev_coul = NULL;
...
PairCoulLongSPH::~PairCoulLongSPH()
{
  ...
  if (allocated) {
    ...
    memory->destroy(dx_rho_rev_coul);
    memory->destroy(dy_rho_rev_coul);
    memory->destroy(dz_rho_rev_coul);
...
void PairCoulLongSPH::compute(int eflag, int vflag)
{
  ...
  if (atom->nmax > nmax) {
    ...
    memory->create(dx_rho_coul,nmax,"pair:dx_rho_coul");
