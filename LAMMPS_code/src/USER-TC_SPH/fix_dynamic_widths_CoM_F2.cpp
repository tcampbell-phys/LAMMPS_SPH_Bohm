...
FixDynamicWidthsCoMFtwo::FixDynamicWidthsCoMFtwo(LAMMPS *lmp, int narg, char **arg):
	Fix(lmp, narg, arg), mu(NULL), mu_all(NULL), tau(NULL), tau_all(NULL)
{
  ...
  assignment_flag = 1;
  fprintf(screen,"\nIn FixDynamicWidthsCoMFtwo with MPI_Allreduce and no commflag = 4 or 5");
}

FixDynamicWidthsCoMFtwo::~FixDynamicWidthsCoMFtwo()
{
...
int FixDynamicWidthsCoMFtwo::setmask()
{
  int mask = 0;
...
void FixDynamicWidthsCoMFtwo::init()
{ 
...
void FixDynamicWidthsCoMFtwo::setup_pre_force(int)
{
  FixedPointIterator();
  CoM_Calculator();
}

void FixDynamicWidthsCoMFtwo::min_pre_force(int)
{
  FixedPointIterator();
  CoM_Calculator();
}
...
void FixDynamicWidthsCoMFtwo::setup_post_neighbor()
{
...
void FixDynamicWidthsCoMFtwo::post_neighbor()
{
  return;
}

void FixDynamicWidthsCoMFtwo::pre_force(int)
{
  FixedPointIterator();
  CoM_Calculator();
}

void FixDynamicWidthsCoMFtwo::FixedPointIterator()
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsquared) {
        ...
        m_gauss_ij = jmass*gauss_pre_i*exp(-(rsq)*hm2_i/2);
        dens_factor_ij = 1. - rho_SPH[i]/rho_SPH[j];
        dx_rho_SPH[i] += ((-delx)*hm2_i)*m_gauss_ij*dens_factor_ij;
        dy_rho_SPH[i] += ((-dely)*hm2_i)*m_gauss_ij*dens_factor_ij;
        dz_rho_SPH[i] += ((-delz)*hm2_i)*m_gauss_ij*dens_factor_ij;
        ...
        if (newton_pair || j < nlocal) {
          ...
          m_gauss_ji = imass*gauss_pre_j*exp(-(rsq)*hm2_j/2);
          dens_factor_ji = 1. - rho_SPH[j]/rho_SPH[i];
          dx_rho_SPH[j] += ((delx)*hm2_j)*m_gauss_ji*dens_factor_ji;
          dy_rho_SPH[j] += ((dely)*hm2_j)*m_gauss_ji*dens_factor_ji;
          dz_rho_SPH[j] += ((delz)*hm2_j)*m_gauss_ji*dens_factor_ji;
        }
      }
    }
  }
...
double FixDynamicWidthsCoMFtwo::Gauss_Width_Deriv(double pre_fact, double wid, double sep_sq)
{
  return pre_fact*exp(-sep_sq/(wid*wid*2.0))*((sep_sq)/(wid*wid*wid) - 3.0/wid);
}

void FixDynamicWidthsCoMFtwo::CoM_Calculator()
{
  int i,ii,inum;
  int *ilist;
	double **x = atom->x;
  ...
  if (unallocated){
    deallocate();
    // fprintf(screen,"\nFixDynamicWidthsCoMFtwo allocating mu tau memory...\n");
    allocate();
    allocated = 1;
    unallocated = 0;
  }
...
int FixDynamicWidthsCoMFtwo::pack_forward_comm(int n, int *list, double *buf,
...
void FixDynamicWidthsCoMFtwo::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
...
int FixDynamicWidthsCoMFtwo::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void FixDynamicWidthsCoMFtwo::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
void FixDynamicWidthsCoMFtwo::allocate()
{
  fprintf(screen,"\nFixDynamicWidthsCoMFtwo::allocate()\n");
...
void FixDynamicWidthsCoMFtwo::deallocate()
{
  fprintf(screen,"\nFixDynamicWidthsCoMFtwo::deallocate()\n");
  delete [] mu;
  delete [] mu_all;
  delete [] tau;
  delete [] tau_all;
