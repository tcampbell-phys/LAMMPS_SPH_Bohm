...
Thomas Campbell
------------------------------------------------------------------------- */
#include "pair_bohm_SPH_dynamic_leg.h"
#include <mpi.h>
#include <cmath>
#include <cstring>
#include <math.h>
...
PairBohmSPHDynamicLeg::PairBohmSPHDynamicLeg(LAMMPS *lmp) : Pair(lmp) {
  nmax = 0;
...
PairBohmSPHDynamicLeg::~PairBohmSPHDynamicLeg()
{
  if (allocated) {
...
void PairBohmSPHDynamicLeg::compute(int eflag, int vflag)
{
...
void PairBohmSPHDynamicLeg::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairBohmSPHDynamicLeg::settings(int narg, char **arg)
{
...
void PairBohmSPHDynamicLeg::coeff(int narg, char **arg)
{
  if (narg < 2 || narg > 3)
...
void PairBohmSPHDynamicLeg::init_style()
{
...
double PairBohmSPHDynamicLeg::init_one(int i, int j)
{
  if (setflag[i][j] == 0)
...
void PairBohmSPHDynamicLeg::write_restart(FILE *fp)
{
...
void PairBohmSPHDynamicLeg::read_restart(FILE *fp)
{
...
void PairBohmSPHDynamicLeg::write_restart_settings(FILE *fp)
{
...
void PairBohmSPHDynamicLeg::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
int PairBohmSPHDynamicLeg::pack_forward_comm(int n, int *list, double *buf,
...
void PairBohmSPHDynamicLeg::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
...
int PairBohmSPHDynamicLeg::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairBohmSPHDynamicLeg::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
void *PairBohmSPHDynamicLeg::extract(const char *str, int &dim)
{
  dim = 2;
  return NULL;
}
