...
PairConfineTest::PairConfineTest(LAMMPS *lmp) : Pair(lmp)
{
...
PairConfineTest::~PairConfineTest()
{
  if (allocated) {
...
void PairConfineTest::compute(int eflag, int vflag)
{
  int i,ii,inum,itype;
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    if (ntimestep < 10){
      if (tagid[i] == 1){
        fprintf(screen,"\nx_COM[i] = %16.16f \nx_COM_use = %16.16f \nx[i][0] = %16.16f \nx[i][0]/N_epe %16.16f \nforce_factor = %16.16f",x_COM[i],x_COM_use,x[i][0],x[i][0]/N_epe,force_factor);
        fprintf(screen,"\ny_COM[i] = %16.16f \ny_COM_use = %16.16f \nx[i][1] = %16.16f \nx[i][1]/N_epe %16.16f \nforce_factor = %16.16f",y_COM[i],y_COM_use,x[i][1],x[i][1]/N_epe,force_factor);
        fprintf(screen,"\nz_COM[i] = %16.16f \nz_COM_use = %16.16f \nx[i][2] = %16.16f \nx[i][2]/N_epe %16.16f \nforce_factor = %16.16f",z_COM[i],z_COM_use,x[i][2],x[i][2]/N_epe,force_factor);
      }
    }
...
void PairConfineTest::settings(int narg, char **arg)
{
...
void PairConfineTest::coeff(int narg, char **arg)
{
...
void PairConfineTest::init_style()
{
...
double PairConfineTest::init_one(int i, int j)
{
  if (setflag[i][j] == 0)
...
void PairConfineTest::write_restart(FILE *fp)
{
...
void PairConfineTest::read_restart(FILE *fp)
{
...
void PairConfineTest::write_restart_settings(FILE *fp)
{
...
void PairConfineTest::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
void *PairConfineTest::extract(const char *str, int &dim)
{
