...
FixNHCOM::FixNHCOM(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg),
  ...
  etap_mass(NULL)
{
  ...
  // // reduce narg to make compatible with reading CoM parameters after while loop
  // narg -= 2;
...
void FixNHCOM::initial_integrate(int /*vflag*/)
{
...
void FixNHCOM::initial_integrate_respa(int /*vflag*/, int ilevel, int /*iloop*/)
{
...
void FixNHCOM::final_integrate_respa(int ilevel, int /*iloop*/)
{
  // set timesteps by level
...
void FixNHCOM::write_restart(FILE *fp)
{
...
int FixNHCOM::size_restart_global()
{
  int nsize = 2;
...
int FixNHCOM::pack_restart_data(double *list)
{
  int n = 0;
...
void FixNHCOM::restart(char *buf)
{
  int n = 0;
...
int FixNHCOM::modify_param(int narg, char **arg)
{
...
double FixNHCOM::compute_scalar()
{
  int i;
  double volume;
  double energy;
...
double FixNHCOM::compute_vector(int n)
{
  int ilen;
...
void FixNHCOM::reset_target(double t_new)
{
...
void *FixNHCOM::extract(const char *str, int &dim)
{
  dim=0;
...
void FixNHCOM::nhc_temp_integrate()
{
  int ich;
  double expfac;
...
void FixNHCOM::nhc_press_integrate()
{
  int ich,i,pdof;
...
double FixNHCOM::compute_strain_energy()
{
...
void FixNHCOM::compute_deviatoric()
{
...
void FixNHCOM::compute_temp_target()
{
...
/* ----------------------------------------------------------------------
   compute Centre of Mass target temperature and kinetic energy
-----------------------------------------------------------------------*/

// void FixNHCOM::compute_temp_target_CoM()
// {
//   double delta = update->ntimestep - update->beginstep;
//   if (delta != 0.0) delta /= update->endstep - update->beginstep;

//   t_target = t_start + delta * (t_stop-t_start);
//   ke_target = tdof * boltz * t_target / N_epe;
// }
...
void FixNHCOM::compute_press_target()
{
  double delta = update->ntimestep - update->beginstep;
  if (delta != 0.0) delta /= update->endstep - update->beginstep;
