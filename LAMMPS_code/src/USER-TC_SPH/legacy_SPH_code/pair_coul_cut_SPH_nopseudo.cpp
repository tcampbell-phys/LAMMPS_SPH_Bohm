...
#include "pair_coul_cut_SPH_nopseudo.h"
#include <mpi.h>
#include <cmath>
#include <cstring>
#include "atom.h"
...
PairCoulCutSPHNoPseudo::PairCoulCutSPHNoPseudo(LAMMPS *lmp) : Pair(lmp) {
  nmax = 0;
...
PairCoulCutSPHNoPseudo::~PairCoulCutSPHNoPseudo()
{
  if (allocated) {
...
void PairCoulCutSPHNoPseudo::compute(int eflag, int vflag)
{
...
void PairCoulCutSPHNoPseudo::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairCoulCutSPHNoPseudo::settings(int narg, char **arg)
{
...
void PairCoulCutSPHNoPseudo::coeff(int narg, char **arg)
{
  if (narg < 2 || narg > 3)
...
void PairCoulCutSPHNoPseudo::init_style()
{
  if (!atom->q_flag)
...
double PairCoulCutSPHNoPseudo::init_one(int i, int j)
{
  if (setflag[i][j] == 0)
...
void PairCoulCutSPHNoPseudo::write_restart(FILE *fp)
{
...
void PairCoulCutSPHNoPseudo::read_restart(FILE *fp)
{
...
void PairCoulCutSPHNoPseudo::write_restart_settings(FILE *fp)
{
...
void PairCoulCutSPHNoPseudo::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
void *PairCoulCutSPHNoPseudo::extract(const char *str, int &dim)
{
  dim = 2;
...
int PairCoulCutSPHNoPseudo::pack_forward_comm(int n, int *list, double *buf,
...
void PairCoulCutSPHNoPseudo::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
...
int PairCoulCutSPHNoPseudo::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairCoulCutSPHNoPseudo::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
