...
EwaldScaleCheck::EwaldScaleCheck(LAMMPS *lmp) : KSpace(lmp),
...
void EwaldScaleCheck::settings(int narg, char **arg)
{
  ...
  fprintf(screen,"\nEwaldCheckScale occupation_fact = %f",occupation_fact);
  fprintf(screen,"\nEwaldCheckScale ele_type = %d",ele_type);
}
...
EwaldScaleCheck::~EwaldScaleCheck()
{
  deallocate();
...
void EwaldScaleCheck::init()
{
  if (comm->me == 0) {
    if (screen) fprintf(screen,"EwaldScaleCheck initialization ...\n");
    if (logfile) fprintf(logfile,"EwaldScaleCheck initialization ...\n");
  }
  ...
  if (domain->dimension == 2)
    error->all(FLERR,"Cannot use EwaldScaleCheck with 2d simulation");

  if (!atom->q_flag) error->all(FLERR,"Kspace style requires atom attribute q");

  if (slabflag == 0 && domain->nonperiodic > 0)
    error->all(FLERR,"Cannot use non-periodic boundaries with EwaldScaleCheck");
  if (slabflag) {
    if (domain->xperiodic != 1 || domain->yperiodic != 1 ||
        domain->boundary[2][0] != 1 || domain->boundary[2][1] != 1)
      error->all(FLERR,"Incorrect boundaries with slab EwaldScaleCheck");
    if (domain->triclinic)
      error->all(FLERR,"Cannot (yet) use EwaldScaleCheck with triclinic box "
                 "and slab correction");
  }
  ...
  // adjust z dimension for 2d slab EwaldScaleCheck
  // 3d EwaldScaleCheck just uses zprd since slab_volfactor = 1.0
  ...
  // setup EwaldScaleCheck coefficients so can print stats
...
/* ----------------------------------------------------------------------
   adjust EwaldScaleCheck coeffs, called initially and whenever volume has changed
------------------------------------------------------------------------- */

void EwaldScaleCheck::setup()
{
  ...
  // adjustment of z dimension for 2d slab EwaldScaleCheck
  // 3d EwaldScaleCheck just uses zprd since slab_volfactor = 1.0
  ...
  // pre-compute EwaldScaleCheck coefficients
...
double EwaldScaleCheck::rms(int km, double prd, bigint natoms, double q2)
{
...
/* ----------------------------------------------------------------------
   compute the EwaldScaleCheck long-range force, energy, virial
------------------------------------------------------------------------- */

void EwaldScaleCheck::compute(int eflag, int vflag)
{
  int i,j,k;
  ...
  for (i = 0; i < nlocal; i++) {
    if (type[i] == ele_type){
      f[i][0] += occupation_fact*qscale * q[i]*ek[i][0];
      f[i][1] += occupation_fact*qscale * q[i]*ek[i][1];
...
void EwaldScaleCheck::eik_dot_r()
{
  int i,k,l,m,n,ic;
...
void EwaldScaleCheck::eik_dot_r_triclinic()
{
  int i,k,l,m,n,ic;
  double cstr1,sstr1;
  double sqk,clpm,slpm;
...
/* ----------------------------------------------------------------------
   pre-compute coefficients for each EwaldScaleCheck K-vector
------------------------------------------------------------------------- */

void EwaldScaleCheck::coeffs()
{
  int k,l,m;
  double sqk,vterm;
...
/* ----------------------------------------------------------------------
   pre-compute coefficients for each EwaldScaleCheck K-vector for a triclinic
   system
------------------------------------------------------------------------- */

void EwaldScaleCheck::coeffs_triclinic()
{
  int k,l,m;
  double sqk,vterm;
...
void EwaldScaleCheck::deallocate()
{
  delete [] kxvecs;
  delete [] kyvecs;
  delete [] kzvecs;
...
/* ----------------------------------------------------------------------
   ...
   periodically repeating slabs.  Yields good approximation to 2D EwaldScaleCheck if
   adequate empty space is left between repeating slabs (J. Chem. Phys.
   111, 3155).  Slabs defined here to be parallel to the xy plane. Also
   extended to non-neutral systems (J. Chem. Phys. 131, 094107).
------------------------------------------------------------------------- */

void EwaldScaleCheck::slabcorr()
{
...
double EwaldScaleCheck::memory_usage()
{
...
/* ----------------------------------------------------------------------
   compute the EwaldScaleCheck total long-range force and energy for groups A and B
 ------------------------------------------------------------------------- */

void EwaldScaleCheck::compute_group_group(int groupbit_A, int groupbit_B, int AA_flag)
{
...
void EwaldScaleCheck::slabcorr_groups(int groupbit_A, int groupbit_B, int AA_flag)
{
  // compute local contribution to global dipole moment
...
void EwaldScaleCheck::allocate_groups()
{
  // group A
...
void EwaldScaleCheck::deallocate_groups()
{
  // group A
