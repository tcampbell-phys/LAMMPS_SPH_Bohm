...
Thomas Campbell
------------------------------------------------------------------------- */
#include "pair_bohm_SPH_dynamic_Mocz_leg.h"
#include <mpi.h>
#include <cmath>
#include <cstring>
#include <math.h>
...
PairBohmSPHDynamicMoczLeg::PairBohmSPHDynamicMoczLeg(LAMMPS *lmp) : Pair(lmp) {
  nmax = 0;
...
PairBohmSPHDynamicMoczLeg::~PairBohmSPHDynamicMoczLeg()
{
  if (allocated) {
...
void PairBohmSPHDynamicMoczLeg::compute(int eflag, int vflag)
{
...
void PairBohmSPHDynamicMoczLeg::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairBohmSPHDynamicMoczLeg::settings(int narg, char **arg)
{
...
void PairBohmSPHDynamicMoczLeg::coeff(int narg, char **arg)
{
  if (narg < 2 || narg > 3)
...
void PairBohmSPHDynamicMoczLeg::init_style()
{
...
double PairBohmSPHDynamicMoczLeg::init_one(int i, int j)
{
  if (setflag[i][j] == 0)
...
void PairBohmSPHDynamicMoczLeg::write_restart(FILE *fp)
{
...
void PairBohmSPHDynamicMoczLeg::read_restart(FILE *fp)
{
...
void PairBohmSPHDynamicMoczLeg::write_restart_settings(FILE *fp)
{
...
void PairBohmSPHDynamicMoczLeg::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
int PairBohmSPHDynamicMoczLeg::pack_forward_comm(int n, int *list, double *buf,
...
void PairBohmSPHDynamicMoczLeg::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
...
int PairBohmSPHDynamicMoczLeg::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairBohmSPHDynamicMoczLeg::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
void *PairBohmSPHDynamicMoczLeg::extract(const char *str, int &dim)
{
  dim = 2;
  return NULL;
}
