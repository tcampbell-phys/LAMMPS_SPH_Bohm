/* ----------------------------------------------------------------------
   ...
   Edited to include option for generating an initial fermi distribution of energy.
   Thomas Campbell 2021.
------------------------------------------------------------------------- */

#include "velocity_fermi.h"
#include <cmath>
#include <cstring>
#include "atom.h"
#include "domain.h"
...
VelocityFermi::VelocityFermi(LAMMPS *lmp) : Pointers(lmp) {}

/* ---------------------------------------------------------------------- */

void VelocityFermi::command(int narg, char **arg)
{
  if (narg < 2) error->all(FLERR,"Illegal VelocityFermi command");

  if (domain->box_exist == 0)
    error->all(FLERR,"VelocityFermi command before simulation box is defined");
  if (atom->natoms == 0)
    error->all(FLERR,"VelocityFermi command with no atoms existing");
  ...
  if (igroup == -1) error->all(FLERR,"Could not find VelocityFermi group ID");
  ...
  else error->all(FLERR,"Illegal VelocityFermi command");
...
/* ----------------------------------------------------------------------
   initialization of defaults before calling VelocityFermi methods externaly
------------------------------------------------------------------------- */

void VelocityFermi::init_external(const char *extgroup)
{
...
void VelocityFermi::create(double t_desired, int seed)
{
  int i;
  double **vhold;

  if (seed <= 0) error->all(FLERR,"Illegal VelocityFermi create command");
  ...
  if (sum_flag) {
    ...
    memory->create(vhold,nlocal,3,"VelocityFermi:vhold");
    for (i = 0; i < nlocal; i++) {
      vhold[i][0] = v[i][0];
      vhold[i][1] = v[i][1];
      vhold[i][2] = v[i][2];
  ...
  // create a new ComputeTemp with the VelocityFermi group
  ...
  if (igroup != temperature->igroup && comm->me == 0)
    error->warning(FLERR,"Mismatch between VelocityFermi and compute groups");
  temperature->init();
  temperature->setup();
  ...
  // if bias_flag set, remove bias VelocityFermi from all atoms
  ...
  // set xdim,ydim,zdim = 1/0 for whether to create VelocityFermi in those dims
  //   zdim = 0 for 2d
  ...
  if (loop_flag == ALL) {
    ...
    if (atom->natoms > MAXSMALLINT)
      error->all(FLERR,"Too big a problem to use VelocityFermi create loop all");
    if (atom->tag_enable == 0)
      error->all(FLERR,
                 "Cannot use VelocityFermi create loop all unless atoms have IDs");
    if (atom->tag_consecutive() == 0)
      error->all(FLERR,
                 "Atom IDs must be consecutive for VelocityFermi create loop all");
  ...
  // if bias_flag set, restore bias VelocityFermi to all atoms
  // reapply needed for temperature computes where VelocityFermi
  //   creation has messed up the bias that was already removed:
  //   compute temp/partial needs to reset v dims to 0.0
  //   compute temp/cs needs to reset v to COM VelocityFermi of each C/S pair
...
void VelocityFermi::set(int /*narg*/, char **arg)
{
  ...
  if (xstr) {
    ...
    if (xvar < 0)
      error->all(FLERR,"Variable name for VelocityFermi set does not exist");
    if (input->variable->equalstyle(xvar)) xstyle = EQUAL;
    else if (input->variable->atomstyle(xvar)) xstyle = ATOM;
    else error->all(FLERR,"Variable for VelocityFermi set is invalid style");
  }
  if (ystr) {
  ...
  if (domain->dimension == 2) {
    if (zstyle == CONSTANT && vz != 0.0)
      error->all(FLERR,"Cannot set non-zero z VelocityFermi for 2d simulation");
    if (zstyle == EQUAL || zstyle == ATOM)
      error->all(FLERR,"Cannot set variable z VelocityFermi for 2d simulation");
  }

  // allocate vfield array if necessary

  double **vfield = NULL;
  if (varflag == ATOM) memory->create(vfield,atom->nlocal,3,"VelocityFermi:vfield");
...
void VelocityFermi::scale(int /*narg*/, char **arg)
{
  double t_desired = force->numeric(FLERR,arg[0]);

  // if temperature = NULL, create a new ComputeTemp with the VelocityFermi group
...
void VelocityFermi::ramp(int /*narg*/, char **arg)
{
  // set scale factors
  ...
  if (v_dim == 2 && domain->dimension == 2)
    error->all(FLERR,"VelocityFermi ramp in z for a 2d problem");
  ...
  // vramp = ramped VelocityFermi component for v_dim
...
void VelocityFermi::zero(int /*narg*/, char **arg)
{
  if (strcmp(arg[0],"linear") == 0) {
    if (rfix < 0) zero_momentum();
      ...
      modify->fix[rfix]->zero_momentum();
    } else error->all(FLERR,"VelocityFermi rigid used with non-rigid fix-ID");
...
void VelocityFermi::rescale(double t_old, double t_new)
{
...
void VelocityFermi::zero_momentum()
{
  ...
  // compute VelocityFermi of center-of-mass of group
...
void VelocityFermi::zero_rotation()
{
  int i;
  ...
  // compute omega (angular VelocityFermi) of group around center-of-mass
...
/* ----------------------------------------------------------------------
   parse optional parameters at end of VelocityFermi input line
------------------------------------------------------------------------- */

void VelocityFermi::options(int narg, char **arg)
{
  if (narg < 0) error->all(FLERR,"Illegal VelocityFermi command");

  int iarg = 0;
  while (iarg < narg) {
    if (strcmp(arg[iarg],"dist") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal VelocityFermi command");
      ...
      if (icompute == modify->ncompute)
        error->all(FLERR,"Could not find VelocityFermi temperature ID");
      temperature = modify->compute[icompute];
      if (temperature->tempflag == 0)
        error->all(FLERR,
                   "VelocityFermi temperature ID does not compute temperature");
      iarg += 2;
      ...
      if (rfix < 0) error->all(FLERR,"Fix ID for VelocityFermi does not exist");
      iarg += 2;
  ...
  if (bias_flag && temperature == NULL)
    error->all(FLERR,"Cannot use VelocityFermi bias command without temp keyword");
  if (bias_flag && temperature->tempbias == 0)
    error->all(FLERR,"VelocityFermi temperature ID does calculate a VelocityFermi bias");
}
