...
enum{ID,MOL,PROC,PROCP1,TYPE,ELEMENT,MASS,
     ...
     TQX,TQY,TQZ,
     COMPUTE,FIX,VARIABLE,INAME,DNAME,BETA_CORRECTION,
     SUM_PACKETS_D,FIRST_ORDER_X_TERM_D,FIRST_ORDER_Y_TERM_D,FIRST_ORDER_Z_TERM_D,
     PHI_D,RHO_X_D,RHO_Y_D,RHO_Z_D,THETA_X_D,THETA_Y_D,THETA_Z_D,SECOND_ORDER_X_TERM_D,
     SECOND_ORDER_Y_TERM_D,SECOND_ORDER_Z_TERM_D,THIRD_ORDER_X_TERM_D,THIRD_ORDER_Y_TERM_D,
     THIRD_ORDER_Z_TERM_D};
  ...
  if (nthresh) {
    ...
    for (int ithresh = 0; ithresh < nthresh; ithresh++) {
      ...
      if (thresh_array[ithresh] == ID) {
        ...
        ptr = atom->q;
        nstride = 1;
      } else if (thresh_array[ithresh] == BETA_CORRECTION) {
        if (!atom->beta_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = atom->beta_correction;
        nstride = 1;
        //custom beta correction
      } else if (thresh_array[ithresh] == SUM_PACKETS_D) {
        if (!atom->forceterms_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = atom->sum_packets_d;
        nstride = 1;
        //custom force term
      } else if (thresh_array[ithresh] == FIRST_ORDER_X_TERM_D) {
        if (!atom->forceterms_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = atom->first_order_x_term_d;
        nstride = 1;
        //custom force term
      } else if (thresh_array[ithresh] == FIRST_ORDER_Y_TERM_D) {
        if (!atom->forceterms_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = atom->first_order_y_term_d;
        nstride = 1;
        //custom force term
      } else if (thresh_array[ithresh] == FIRST_ORDER_Z_TERM_D) {
        if (!atom->forceterms_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = atom->first_order_z_term_d;
        nstride = 1;
        //custom force term
      } else if (thresh_array[ithresh] == PHI_D) {
        if (!atom->forceterms_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = atom->phi_d;
        nstride = 1;
        //custom force term
      } else if (thresh_array[ithresh] == RHO_X_D) {
        if (!atom->forceterms_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = atom->rho_x_d;
        nstride = 1;
        //custom force term
      } else if (thresh_array[ithresh] == RHO_Y_D) {
        if (!atom->forceterms_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = atom->rho_y_d;
        ...
        nstride = 1;
        //custom force term
      } else if (thresh_array[ithresh] == THETA_X_D) {
        if (!atom->forceterms_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = atom->theta_x_d;
        nstride = 1;
        //custom force term
      } else if (thresh_array[ithresh] == THETA_Y_D) {
        if (!atom->forceterms_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = atom->theta_y_d;
        nstride = 1;
        //custom force term
      } else if (thresh_array[ithresh] == THETA_Z_D) {
        if (!atom->forceterms_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = atom->theta_z_d;
        nstride = 1;
        //custom force term
      } else if (thresh_array[ithresh] == SECOND_ORDER_X_TERM_D) {
        if (!atom->forceterms_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = atom->second_order_x_term_d;
        nstride = 1;
        //custom force term
      } else if (thresh_array[ithresh] == SECOND_ORDER_Y_TERM_D) {
        if (!atom->forceterms_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = atom->second_order_y_term_d;
        nstride = 1;
        //custom force term
      } else if (thresh_array[ithresh] == SECOND_ORDER_Z_TERM_D) {
        if (!atom->forceterms_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = atom->second_order_z_term_d;
        nstride = 1;
        //custom force term
      } else if (thresh_array[ithresh] == THIRD_ORDER_X_TERM_D) {
        if (!atom->forceterms_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = atom->third_order_x_term_d;
        nstride = 1;
        //custom force term
      } else if (thresh_array[ithresh] == THIRD_ORDER_Y_TERM_D) {
        if (!atom->forceterms_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = atom->third_order_y_term_d;
        nstride = 1;
        //custom force term
      } else if (thresh_array[ithresh] == THIRD_ORDER_Z_TERM_D) {
        if (!atom->forceterms_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = atom->third_order_z_term_d;
        nstride = 1;
        //custom force term
...
int DumpCustom::parse_fields(int narg, char **arg)
{
  ...
  for (int iarg = 0; iarg < narg; iarg++) {
    ...
    if (strcmp(arg[iarg],"id") == 0) {
      ...
      vtype[i] = Dump::DOUBLE;
    } else if (strcmp(arg[iarg],"beta_correction") == 0) {
      if (!atom->beta_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_beta_correction;
      vtype[i] = Dump::DOUBLE;
      //custom beta correction
    } else if (strcmp(arg[iarg],"sum_packets_d") == 0) {
      if (!atom->forceterms_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_sum_packets_d;
      vtype[i] = Dump::DOUBLE;
      //custom force term correction
    } else if (strcmp(arg[iarg],"first_order_x_term_d") == 0) {
      if (!atom->forceterms_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_first_order_x_term_d;
      vtype[i] = Dump::DOUBLE;
      //custom force term correction
    } else if (strcmp(arg[iarg],"first_order_y_term_d") == 0) {
      if (!atom->forceterms_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_first_order_y_term_d;
      vtype[i] = Dump::DOUBLE;
      //custom force term correction
    } else if (strcmp(arg[iarg],"first_order_z_term_d") == 0) {
      if (!atom->forceterms_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_first_order_z_term_d;
      vtype[i] = Dump::DOUBLE;
      //custom force term correction
    } else if (strcmp(arg[iarg],"phi_d") == 0) {
      if (!atom->forceterms_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_phi_d;
      vtype[i] = Dump::DOUBLE;
      //custom force term correction
    } else if (strcmp(arg[iarg],"rho_x_d") == 0) {
      if (!atom->forceterms_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_rho_x_d;
      vtype[i] = Dump::DOUBLE;
      //custom force term correction
    } else if (strcmp(arg[iarg],"rho_y_d") == 0) {
      if (!atom->forceterms_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_rho_y_d;
      vtype[i] = Dump::DOUBLE;
      //custom force term correction
    } else if (strcmp(arg[iarg],"rho_z_d") == 0) {
      if (!atom->forceterms_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_rho_z_d;
      vtype[i] = Dump::DOUBLE;
      //custom force term correction
    } else if (strcmp(arg[iarg],"theta_x_d") == 0) {
      if (!atom->forceterms_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_theta_x_d;
      vtype[i] = Dump::DOUBLE;
      //custom force term correction
    } else if (strcmp(arg[iarg],"theta_y_d") == 0) {
      if (!atom->forceterms_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_theta_y_d;
      vtype[i] = Dump::DOUBLE;
      //custom force term correction
    } else if (strcmp(arg[iarg],"theta_z_d") == 0) {
      if (!atom->forceterms_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_theta_z_d;
      vtype[i] = Dump::DOUBLE;
      //custom force term correction
    } else if (strcmp(arg[iarg],"second_order_x_term_d") == 0) {
      if (!atom->forceterms_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_second_order_x_term_d;
      vtype[i] = Dump::DOUBLE;
      //custom force term correction
    } else if (strcmp(arg[iarg],"second_order_y_term_d") == 0) {
      if (!atom->forceterms_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_second_order_y_term_d;
      vtype[i] = Dump::DOUBLE;
      //custom force term correction
    } else if (strcmp(arg[iarg],"second_order_z_term_d") == 0) {
      if (!atom->forceterms_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_second_order_z_term_d;
      vtype[i] = Dump::DOUBLE;
      //custom force term correction
    } else if (strcmp(arg[iarg],"third_order_x_term_d") == 0) {
      if (!atom->forceterms_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_third_order_x_term_d;
      vtype[i] = Dump::DOUBLE;
      //custom force term correction
    } else if (strcmp(arg[iarg],"third_order_y_term_d") == 0) {
      if (!atom->forceterms_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_third_order_y_term_d;
      vtype[i] = Dump::DOUBLE;
      //custom force term correction
    } else if (strcmp(arg[iarg],"third_order_z_term_d") == 0) {
      if (!atom->forceterms_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_third_order_z_term_d;
      vtype[i] = Dump::DOUBLE;
      //custom force term correction
...
int DumpCustom::modify_param(int narg, char **arg)
{
  ...
  if (strcmp(arg[0],"thresh") == 0) {
    ...
    else if (strcmp(arg[1],"beta_correction") == 0) thresh_array[nthresh] = BETA_CORRECTION;
    //custom beta correction
    else if (strcmp(arg[1],"sum_packets_d") == 0) thresh_array[nthresh] = SUM_PACKETS_D;
    else if (strcmp(arg[1],"first_order_x_term_d") == 0) thresh_array[nthresh] = FIRST_ORDER_X_TERM_D;
    else if (strcmp(arg[1],"first_order_y_term_d") == 0) thresh_array[nthresh] = FIRST_ORDER_Y_TERM_D;
    else if (strcmp(arg[1],"first_order_z_term_d") == 0) thresh_array[nthresh] = FIRST_ORDER_Z_TERM_D;
    else if (strcmp(arg[1],"phi_d") == 0) thresh_array[nthresh] = PHI_D;
    else if (strcmp(arg[1],"rho_x_d") == 0) thresh_array[nthresh] = RHO_X_D;
    else if (strcmp(arg[1],"rho_y_d") == 0) thresh_array[nthresh] = RHO_Y_D;
    else if (strcmp(arg[1],"rho_z_d") == 0) thresh_array[nthresh] = RHO_Z_D;
    else if (strcmp(arg[1],"theta_x_d") == 0) thresh_array[nthresh] = THETA_X_D;
    else if (strcmp(arg[1],"theta_y_d") == 0) thresh_array[nthresh] = THETA_Y_D;
    else if (strcmp(arg[1],"theta_z_d") == 0) thresh_array[nthresh] = THETA_Z_D;
    else if (strcmp(arg[1],"second_order_x_term_d") == 0) thresh_array[nthresh] = SECOND_ORDER_X_TERM_D;
    else if (strcmp(arg[1],"second_order_y_term_d") == 0) thresh_array[nthresh] = SECOND_ORDER_Y_TERM_D;
    else if (strcmp(arg[1],"second_order_z_term_d") == 0) thresh_array[nthresh] = SECOND_ORDER_Z_TERM_D;
    else if (strcmp(arg[1],"third_order_x_term_d") == 0) thresh_array[nthresh] = THIRD_ORDER_X_TERM_D;
    else if (strcmp(arg[1],"third_order_y_term_d") == 0) thresh_array[nthresh] = THIRD_ORDER_Y_TERM_D;
    else if (strcmp(arg[1],"third_order_z_term_d") == 0) thresh_array[nthresh] = THIRD_ORDER_Z_TERM_D;
    //custom force terms
...
void DumpCustom::pack_beta_correction(int n)
{
  double *beta_correction = atom->beta_correction;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = beta_correction[clist[i]];
    n += size_one;
  }
}
...
void DumpCustom::pack_sum_packets_d(int n)
{
  double *sum_packets_d = atom->sum_packets_d;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = sum_packets_d[clist[i]];
    n += size_one;
  }
}
...
void DumpCustom::pack_first_order_x_term_d(int n)
{
  double *first_order_x_term_d = atom->first_order_x_term_d;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = first_order_x_term_d[clist[i]];
    n += size_one;
  }
}
...
void DumpCustom::pack_first_order_y_term_d(int n)
{
  double *first_order_y_term_d = atom->first_order_y_term_d;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = first_order_y_term_d[clist[i]];
    n += size_one;
  }
}
...
void DumpCustom::pack_first_order_z_term_d(int n)
{
  double *first_order_z_term_d = atom->first_order_z_term_d;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = first_order_z_term_d[clist[i]];
    n += size_one;
  }
}
...
void DumpCustom::pack_phi_d(int n)
{
...
void DumpCustom::pack_rho_x_d(int n)
{
  double *rho_x_d = atom->rho_x_d;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = rho_x_d[clist[i]];
    n += size_one;
  }
}
...
void DumpCustom::pack_rho_y_d(int n)
{
  double *rho_y_d = atom->rho_y_d;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = rho_y_d[clist[i]];
    n += size_one;
  }
}
...
void DumpCustom::pack_rho_z_d(int n)
{
  double *rho_z_d = atom->rho_z_d;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = rho_z_d[clist[i]];
    n += size_one;
  }
}
...
void DumpCustom::pack_theta_x_d(int n)
{
  double *theta_x_d = atom->theta_x_d;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = theta_x_d[clist[i]];
    n += size_one;
  }
}
...
void DumpCustom::pack_theta_y_d(int n)
{
  double *theta_y_d = atom->theta_y_d;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = theta_y_d[clist[i]];
    n += size_one;
  }
}
...
void DumpCustom::pack_theta_z_d(int n)
{
  double *theta_z_d = atom->theta_z_d;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = theta_z_d[clist[i]];
    n += size_one;
  }
}
...
void DumpCustom::pack_second_order_x_term_d(int n)
{
  double *second_order_x_term_d = atom->second_order_x_term_d;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = second_order_x_term_d[clist[i]];
    n += size_one;
  }
}
...
void DumpCustom::pack_second_order_y_term_d(int n)
{
  double *second_order_y_term_d = atom->second_order_y_term_d;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = second_order_y_term_d[clist[i]];
    n += size_one;
  }
}
...
void DumpCustom::pack_second_order_z_term_d(int n)
{
  double *second_order_z_term_d = atom->second_order_z_term_d;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = second_order_z_term_d[clist[i]];
    n += size_one;
  }
}
...
void DumpCustom::pack_third_order_x_term_d(int n)
{
  double *third_order_x_term_d = atom->third_order_x_term_d;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = third_order_x_term_d[clist[i]];
    n += size_one;
  }
}
...
void DumpCustom::pack_third_order_y_term_d(int n)
{
  double *third_order_y_term_d = atom->third_order_y_term_d;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = third_order_y_term_d[clist[i]];
    n += size_one;
  }
}
...
void DumpCustom::pack_third_order_z_term_d(int n)
{
  double *third_order_z_term_d = atom->third_order_z_term_d;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = third_order_z_term_d[clist[i]];
    n += size_one;
  }
}
