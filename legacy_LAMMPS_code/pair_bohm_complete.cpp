...
void PairBohmGlobalIons::allocate()
{
  ...
  // create array for defining different core electron widths (arbitrary maximum size provided it is large enough)

  memory->create(ion_widths,20,"pair:ion_widths");

  // create array for defining different core electron width occupancies

  memory->create(ion_width_occups,20,"pair:ion_width_occups");
}
...
void PairBohmGlobalIons::settings(int narg, char **arg)
{
  ...
  ion_core_electrons = force->numeric(FLERR,arg[2]);
  gamma_factor = force->numeric(FLERR,arg[3]);
  n_ion_widths = force->numeric(FLERR,arg[4]);

  fprintf(screen,"gamma_factor = %f \n",gamma_factor);
  fprintf(screen,"n_ion_widths = %f \n",n_ion_widths);
  fprintf(screen,"sigma_global = %f \n",sigma_global);

  for (int i = 1; i <= n_ion_widths; i++)
    ion_widths[i] = force->numeric(FLERR,arg[4+i]);
    ion_width_occups[i] = force->numeric(FLERR,arg[4+n_ion_widths+i]);
    fprintf(screen,"ion_width = %f \n",ion_widths[i]);
    fprintf(screen,"ion_width_occup = %f \n",ion_width_occups[i]);
  ...
  e_packet_pre = 1/(pow(2*M_PI,3/2)*e_sigma_3);
  ion_packet_pre = ion_core_electrons/(pow(2*M_PI,3/2)*ion_sigma_3);
