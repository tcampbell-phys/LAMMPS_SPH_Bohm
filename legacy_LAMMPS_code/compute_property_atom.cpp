  ...
  for (int iarg = 3; iarg < narg; iarg++) {
    ...
    if (strcmp(arg[iarg],"id") == 0) {
      ...
      if (!atom->beta_flag)
        error->all(FLERR,"Compute property/atom for "
                   "atom property that isn't allocated");
      pack_choice[i] = &ComputePropertyAtom::pack_beta_correction;
      //custom beta_correction
    } else if (strcmp(arg[iarg],"sum_packets_d") == 0) {
      if (!atom->forceterms_flag)
        error->all(FLERR,"Compute property/atom for "
                   "atom property that isn't allocated");
      pack_choice[i] = &ComputePropertyAtom::pack_sum_packets_d;
      ...
      if (!atom->forceterms_flag)
        error->all(FLERR,"Compute property/atom for "
                   "atom property that isn't allocated");
      pack_choice[i] = &ComputePropertyAtom::pack_first_order_x_term_d;
      ...
      if (!atom->forceterms_flag)
        error->all(FLERR,"Compute property/atom for "
                   "atom property that isn't allocated");
      pack_choice[i] = &ComputePropertyAtom::pack_first_order_y_term_d;
      ...
      if (!atom->forceterms_flag)
        error->all(FLERR,"Compute property/atom for "
                   "atom property that isn't allocated");
      pack_choice[i] = &ComputePropertyAtom::pack_first_order_z_term_d;
      ...
      if (!atom->forceterms_flag)
        error->all(FLERR,"Compute property/atom for "
                   "atom property that isn't allocated");
      pack_choice[i] = &ComputePropertyAtom::pack_phi_d;
      ...
      if (!atom->forceterms_flag)
        error->all(FLERR,"Compute property/atom for "
                   "atom property that isn't allocated");
      pack_choice[i] = &ComputePropertyAtom::pack_rho_x_d;
      ...
      if (!atom->forceterms_flag)
        error->all(FLERR,"Compute property/atom for "
                   "atom property that isn't allocated");
      pack_choice[i] = &ComputePropertyAtom::pack_rho_y_d;
      ...
      if (!atom->forceterms_flag)
        error->all(FLERR,"Compute property/atom for "
                   "atom property that isn't allocated");
      pack_choice[i] = &ComputePropertyAtom::pack_rho_z_d;
      ...
      if (!atom->forceterms_flag)
        error->all(FLERR,"Compute property/atom for "
                   "atom property that isn't allocated");
      pack_choice[i] = &ComputePropertyAtom::pack_theta_x_d;
      ...
      if (!atom->forceterms_flag)
        error->all(FLERR,"Compute property/atom for "
                   "atom property that isn't allocated");
      pack_choice[i] = &ComputePropertyAtom::pack_theta_y_d;
      ...
      if (!atom->forceterms_flag)
        error->all(FLERR,"Compute property/atom for "
                   "atom property that isn't allocated");
      pack_choice[i] = &ComputePropertyAtom::pack_theta_z_d;
      ...
      if (!atom->forceterms_flag)
        error->all(FLERR,"Compute property/atom for "
                   "atom property that isn't allocated");
      pack_choice[i] = &ComputePropertyAtom::pack_second_order_x_term_d;
      ...
      if (!atom->forceterms_flag)
        error->all(FLERR,"Compute property/atom for "
                   "atom property that isn't allocated");
      pack_choice[i] = &ComputePropertyAtom::pack_second_order_y_term_d;
      ...
      if (!atom->forceterms_flag)
        error->all(FLERR,"Compute property/atom for "
                   "atom property that isn't allocated");
      pack_choice[i] = &ComputePropertyAtom::pack_second_order_z_term_d;
      ...
      if (!atom->forceterms_flag)
        error->all(FLERR,"Compute property/atom for "
                   "atom property that isn't allocated");
      pack_choice[i] = &ComputePropertyAtom::pack_third_order_x_term_d;
      ...
      if (!atom->forceterms_flag)
        error->all(FLERR,"Compute property/atom for "
                   "atom property that isn't allocated");
      pack_choice[i] = &ComputePropertyAtom::pack_third_order_y_term_d;
      ...
      if (!atom->forceterms_flag)
        error->all(FLERR,"Compute property/atom for "
                   "atom property that isn't allocated");
      pack_choice[i] = &ComputePropertyAtom::pack_third_order_z_term_d;
...
void ComputePropertyAtom::pack_beta_correction(int n)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) buf[n] = beta_correction[i];
    else buf[n] = 0.0;
    n += nvalues;
  }
}
...
void ComputePropertyAtom::pack_sum_packets_d(int n)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) buf[n] = sum_packets_d[i];
    else buf[n] = 0.0;
    n += nvalues;
  }
}
...
void ComputePropertyAtom::pack_first_order_x_term_d(int n)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) buf[n] = first_order_x_term_d[i];
    else buf[n] = 0.0;
    n += nvalues;
  }
}
...
void ComputePropertyAtom::pack_first_order_y_term_d(int n)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) buf[n] = first_order_y_term_d[i];
    else buf[n] = 0.0;
    n += nvalues;
  }
}
...
void ComputePropertyAtom::pack_first_order_z_term_d(int n)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) buf[n] = first_order_z_term_d[i];
    else buf[n] = 0.0;
    n += nvalues;
  }
}
...
void ComputePropertyAtom::pack_phi_d(int n)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) buf[n] = phi_d[i];
    else buf[n] = 0.0;
    n += nvalues;
  }
}
...
void ComputePropertyAtom::pack_rho_x_d(int n)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) buf[n] = rho_x_d[i];
    else buf[n] = 0.0;
    n += nvalues;
  }
}
...
void ComputePropertyAtom::pack_rho_y_d(int n)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) buf[n] = rho_y_d[i];
    else buf[n] = 0.0;
    n += nvalues;
  }
}
...
void ComputePropertyAtom::pack_rho_z_d(int n)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) buf[n] = rho_z_d[i];
    else buf[n] = 0.0;
    n += nvalues;
  }
}
...
void ComputePropertyAtom::pack_theta_x_d(int n)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) buf[n] = theta_x_d[i];
    else buf[n] = 0.0;
    n += nvalues;
  }
}
...
void ComputePropertyAtom::pack_theta_y_d(int n)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) buf[n] = theta_y_d[i];
    else buf[n] = 0.0;
    n += nvalues;
  }
}
...
void ComputePropertyAtom::pack_theta_z_d(int n)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) buf[n] = theta_z_d[i];
    else buf[n] = 0.0;
    n += nvalues;
  }
}
...
void ComputePropertyAtom::pack_second_order_x_term_d(int n)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) buf[n] = second_order_x_term_d[i];
    else buf[n] = 0.0;
    n += nvalues;
  }
}
...
void ComputePropertyAtom::pack_second_order_y_term_d(int n)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) buf[n] = second_order_y_term_d[i];
    else buf[n] = 0.0;
    n += nvalues;
  }
}
...
void ComputePropertyAtom::pack_second_order_z_term_d(int n)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) buf[n] = second_order_z_term_d[i];
    else buf[n] = 0.0;
    n += nvalues;
  }
}
...
void ComputePropertyAtom::pack_third_order_x_term_d(int n)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) buf[n] = third_order_x_term_d[i];
    else buf[n] = 0.0;
    n += nvalues;
  }
}
...
void ComputePropertyAtom::pack_third_order_y_term_d(int n)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) buf[n] = third_order_y_term_d[i];
    else buf[n] = 0.0;
    n += nvalues;
  }
}
...
void ComputePropertyAtom::pack_third_order_z_term_d(int n)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) buf[n] = third_order_z_term_d[i];
    else buf[n] = 0.0;
    n += nvalues;
  }
}
